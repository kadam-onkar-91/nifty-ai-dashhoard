...
def _get_prev_close(yf_ticker):
    """Fetches the LAST COMPLETED trading day's close from Yahoo Finance.
    We deliberately don't trust Upstox's own ohlc.close for this: once the
    market closes for the day, Upstox starts returning TODAY's own closing
    price in that field (same as the live price), which would make every
    change_pct compute as a false 0.0% after-hours -- even though the index
    genuinely moved during the session. Using an independent previous-close
    source keeps change_pct correct at all times, live or after close."""
    try:
...
def _fetch_upstox_index(access_token, instrument_key, prev_close):
    if not access_token or prev_close is None or prev_close == 0:
        return None
    try:
        import urllib.parse
        ...
        if ltp is None:
            return None
        # prev_close comes from Yahoo (see _get_prev_close), NOT from
        # Upstox's own ohlc.close -- that field is unreliable after
        # market hours (see docstring above).
...
def get_bank_nifty_sensex(access_token=None, nifty_change_pct=None):
    ...
    for target in INDEX_TARGETS:
        prev_close = _get_prev_close(target["yf"])
        value = _fetch_upstox_index(access_token, target["instrument_key"], prev_close)
        source = "Upstox (Live)"
        if value is None:
