...
def _get_index_prev_close(ticker):
    """Same fix as market_breadth.py: Upstox's live ohlc.close mirrored
    last_price (0.00% for everything), so we use Yahoo's prior daily close
    as the unambiguous reference instead, and keep Upstox's LTP as the
    live current price."""
    try:
...
def _fetch_upstox_index(access_token, instrument_key, yf_ticker):
    ...
    try:
        ...
        prev_close = _get_index_prev_close(yf_ticker)
...
def get_bank_nifty_sensex(access_token=None, nifty_change_pct=None):
    ...
    for target in INDEX_TARGETS:
        value = _fetch_upstox_index(access_token, target["instrument_key"], target["yf"])
        source = "Upstox (Live)"
        if value is None:
