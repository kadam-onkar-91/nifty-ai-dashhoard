...
def _walk_forward_folds(X, y, n_folds=4, embargo=5):
    """Expanding-window walk-forward validation with a label embargo.

    The last `embargo` training rows are removed before each test window
    because their forward-looking labels can inspect candles inside the test
    period. This prevents target leakage across the train/test boundary.
    """
    total = len(X)
    ...
    for i in range(1, n_folds + 1):
        ...
        train_cut = max(0, train_end - int(embargo))
        if test_end <= train_cut or train_cut < 10:
            continue
        folds.append((
            X.iloc[:train_cut], y.iloc[:train_cut],
...
def train_and_backtest(df, live_vix=None):
    ...
    empty_result = {
        ...
        "latest_confidence": 0.0, "calibrated_confidence": None,
        "calibration_note": "Not enough data yet"
    }
    ...
    # The final `holding_period` rows do not have enough future candles to
    # receive a genuine label. Mark them unknown instead of incorrectly
    # treating the placeholder 0 as a real neutral outcome.
    ...
    folds = _walk_forward_folds(X, y, n_folds=4, embargo=holding_period)
    if not folds:
        return empty_result

    all_final_preds, all_actuals = [], []
    all_raw_confidences, all_raw_correct = [], []
    for X_train, y_train, X_test, y_test in folds:
        ...
        for row_idx in range(len(X_test)):
            ...
            # Raw (pre-threshold) confidence vs correctness -- this is the
            # real signal calibration is built from further down. Collected
            # BEFORE the MIN_CONFIDENCE override below, since calibration
            # needs to know how reliable a given raw confidence number
            # actually is, independent of the trading threshold applied to it.
            all_raw_confidences.append(float(best_conf))
            all_raw_correct.append(1 if predicted_class == actual else 0)
    ...
    # -------------------------------------------------------------
    # PHASE 2 — Confidence Calibration (real, from walk-forward OOS data)
    # Fits an isotonic regression mapping "raw ensemble confidence" ->
    # "empirical probability of being correct", using ONLY the real
    # out-of-sample predictions collected above. This answers a genuinely
    # different question than raw accuracy: "when the model SAYS 65%
    # confident, how often is it actually right?" -- models are very
    # commonly over- or under-confident, and this is the honest check for
    # that, not a cosmetic number. Needs a reasonable sample to be
    # meaningful; below that it honestly reports uncalibrated.
    # -------------------------------------------------------------
    calibrator = None
    calibration_note = "Not enough OOS data yet for calibration"
    MIN_CALIBRATION_SAMPLES = 15
    if len(all_raw_confidences) >= MIN_CALIBRATION_SAMPLES:
        try:
            calibrator = IsotonicRegression(out_of_bounds='clip', y_min=0.0, y_max=1.0)
            calibrator.fit(all_raw_confidences, all_raw_correct)
            calibration_note = f"Calibrated on {len(all_raw_confidences)} real walk-forward OOS predictions"
        except Exception:
            logger.exception("Broad exception caught; fallback path executed")
            calibrator = None
            calibration_note = "Calibration fit failed"
        ...
        if y.nunique() >= 2:
            ...
            if proba is not None:
                ...
                if calibrator is not None:
                    calibrated_confidence = round(float(calibrator.predict([latest_confidence])[0]), 3)
