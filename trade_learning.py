...
ALL_FACTOR_KEYS = [
    ...
    "order_flow_aligned", "regime_aligned", "mtf_aligned",
]
...
FACTOR_LABELS = {
    ...
    "order_flow_aligned": "Live NIFTY futures order-book pressure aligned",
    "regime_aligned": "Market regime supports the trade direction",
    "mtf_aligned": "Higher/lower timeframe structure aligned",
}
...
def _resolved_learning_rows():
    ...
    """
    try:
        return _sb_resolved_factors_status() if _is_supabase_configured() else _sqlite_resolved_factors_status()
    except Exception as e:
        print(f"[trade_learning] learning read failed, using local fallback: {e}")
        return _sqlite_resolved_factors_status()
...
def compute_confidence(factor_flags: dict):
    """Return a bounded confidence estimate using weighted confluence + learning.

    Factors are evidence, not a mandatory checklist.  Safety gates live in
    ai_trade_decision.py; this function only scores a candidate that already
    passed those gates.  Correlated factors are deliberately capped.
    """
    rows = _resolved_learning_rows()

    # More weight goes to direct price/structure evidence.  Contextual factors
    # (news/global/VIX/etc.) can strengthen a setup but cannot dominate it.
    weights = {
        "main_signal_aligned": 2.0, "level_pct_ge_65": 1.8,
        "htf_1h_aligned": 1.6, "htf_15min_aligned": 1.4,
        "regime_aligned": 1.6, "mtf_aligned": 1.5,
        "order_flow_aligned": 1.5, "vwap_aligned": 1.3,
        "liquidity_sweep": 1.3, "fvg_ob_confluence": 1.3,
        "oi_aligned": 1.2, "ladder_confluence_aligned": 1.1,
        "sniper_setup_aligned": 1.1, "breadth_aligned": 0.9,
        "banknifty_no_divergence": 0.8, "global_aligned": 0.7,
        "news_sentiment_aligned": 0.6, "away_from_max_pain": 0.5,
        "low_vix": 0.4, "round_number_level": 0.3, "ml_agrees": 1.0,
    }
    active = [k for k in ALL_FACTOR_KEYS if factor_flags.get(k)]
    total_w = sum(weights.get(k, 1.0) for k in ALL_FACTOR_KEYS)
    active_w = sum(weights.get(k, 1.0) for k in active)
    coverage = active_w / total_w if total_w else 0.0

    # Base score spans roughly 50-74 before learned adjustment.
    base = 45.0 + 40.0 * coverage
    ...
    for key in active:
        ...
        if p_true is None or n_true < MIN_SAMPLES_FOR_LEARNING:
            continue
        # Compare the factor's observed win rate to the observed non-factor
        # baseline; keep the correction small to avoid overfitting.
        ...
        lift = max(-0.12, min(0.12, p_true - baseline))
        learned.append(lift * min(weights.get(key, 1.0), 1.6))

    if learned:
        base += (sum(learned) / len(learned)) * 22.0

    confidence = max(CONFIDENCE_FLOOR, min(CONFIDENCE_CEILING, round(base, 1)))
    return confidence, bool(learned), len(learned)
