...
def get_recent_setups(limit=20):
    """
    NEW — real, visible history of every setup this engine has actually
    logged (via the "Log this setup" button), newest first, whatever
    its current status (OPEN / WIN / LOSS / EXPIRED). This answers
    "mujhe live trade dikhayen kab liya tha" directly and honestly --
    if this list is empty, no setup has been logged, period.
    """
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
