...
def compute_confidence(factor_flags: dict, regime=None):
    ...
    """
    rows = _resolved_learning_rows()
    # Regime metadata is stored inside factors_json, so this remains backward
    # compatible with the existing Supabase schema and old trades.
    if regime:
        regime_rows = []
        for _fj, _st in rows:
            try:
                _f = json.loads(_fj) if isinstance(_fj, str) else (_fj or {})
                if _f.get("_meta_regime") == regime:
                    regime_rows.append((_fj, _st))
            except Exception:
                continue
        # Only switch to regime-specific history when there is enough data;
        # otherwise use the broader sample rather than overfitting a tiny set.
        if len(regime_rows) >= MIN_SAMPLES_FOR_LEARNING:
            rows = regime_rows
    weighted = []
