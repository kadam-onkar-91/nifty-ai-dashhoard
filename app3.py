...
# Native auto-refresh (no external package). A plain HTML meta-refresh
# reloads the whole browser page, which wipes st.session_state (logging
# Upstox out every refresh) -- so this uses st.fragment(run_every=...)
# instead, which reruns the app over the SAME session/websocket. IMPORTANT:
# a run_every fragment executes once immediately on the very first script
# run too -- calling st.rerun() unconditionally there would loop before
# anything ever renders (blank page forever). The _app_booted guard below
# skips the very first execution so the page actually renders once before
# the periodic refresh kicks in.
...
if df is not None and not df.empty:
    ...
    with col1:
        delta_str = f"{nifty_change_pct:+.2f}%" if nifty_change_pct is not None else None
        st.metric(label="Real-Time Live Nifty Price (LTP)", value=f"₹ {live_price:,.2f}", delta=delta_str)
    with col2:
