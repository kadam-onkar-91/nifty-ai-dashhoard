...
def fetch_all_closed_trades():
    """
    PHASE 3 addition. Returns every resolved (non-OPEN) trade as a list of
    dicts, ordered oldest-first -- this is the single real source of truth
    that backtest_engine.py, monte_carlo.py and drift_detection.py build
    on. No synthetic/simulated trades are ever mixed in here; if the list
    is short, those modules are expected to say so plainly rather than
    pad it with invented data.
    """
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("SELECT count(name) FROM sqlite_master WHERE type='table' AND name='trades'")
