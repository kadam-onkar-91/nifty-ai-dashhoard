...
if df is not None and not df.empty:
    ...
    df_full50, adv50, dec50, ratio50, breadth50_status = market_breadth.get_full_nifty50_breadth()
    ...
    if breadth50_status.startswith("DATA UNAVAILABLE"):
        ...
        st.info(f"**Full Breadth Status:** {breadth50_status} — Source: Yahoo Finance (delayed ~15-20 min)")
        with st.expander("📋 View all 50 stocks (sorted by change %)", expanded=False):
            try:
                st.dataframe(df_full50.style.background_gradient(subset=['Change (%)'], cmap='RdYlGn'),
                             use_container_width=True, hide_index=True)
            except Exception:
                st.dataframe(df_full50, use_container_width=True, hide_index=True)
