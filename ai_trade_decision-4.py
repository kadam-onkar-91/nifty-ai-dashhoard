...
def _build_trade_levels(cand, X):
    ...
    # ROOM-TO-RUN CHECK: `tgts[0]` (if any) is the nearest real opposing-side
    # level, whether or not it cleared the MIN_RR filter above. That level is
    # a wall the trade still has to get through. Previously the ATR-based
    # fallback target was placed FALLBACK_RR*sl_dist away regardless of this
    # wall, so a bounce/rejection trade could get a "target" sitting PAST an
    # un-broken opposing support/resistance that was only 15-20 pts away --
    # the trade then neither hit that unrealistic target nor a clean SL, and
    # mostly just EXPIRED or reversed off the real wall into the stop. The
    # fallback now stops short of a real nearby wall instead of walking
    # through it.
    nearest_wall = tgts[0] if tgts else None
