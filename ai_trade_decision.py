...
def generate_trade_decision(live_price, level_prediction, atr, max_pain=None,
    ...
    level_ok = level_pct >= 50

    flags = {
        "main_signal_aligned": True, "level_evidence_ok": level_ok, "ml_agrees": ml_ok,
    ...
    weights = {
        "main_signal_aligned": 12, "level_evidence_ok": 8, "ml_agrees": 7, "banknifty_no_divergence": 5,
    ...
    # Single unified entry model: early evidence and confirmation are scored together.
    # Confirmation improves the score but is NOT a separate mandatory entry gate.
    critical = flags["main_signal_aligned"] and flags["level_evidence_ok"]
    if not critical:
        return {"has_setup": False, "reason": f"Nearby level evidence is weak ({level_pct:.1f}%).", "score": round(score,1), "factor_flags": flags, "context_audit": audit}
    # One entry path: take the trade when the combined evidence is strong enough.
    # This intentionally avoids waiting for a second confirmation stage.
    if score < 64:
        return {"has_setup": False, "reason": f"Combined trade evidence {score:.1f}/100 is below the entry threshold.", "score": round(score,1), "factor_flags": flags, "context_audit": audit}
