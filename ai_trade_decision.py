...
def generate_trade_decision(live_price, level_prediction, atr, max_pain=None,
    ...
    if path_strong:
        max_entry_distance = max((0.65 if breakout_entry else 0.50) * float(atr),
                                 10.0 if breakout_entry else 8.0)
    elif fast_path_active:
        max_entry_distance = max(0.55 * float(atr), 9.0)
    else:
    ...
    # Fast structural rejection/bounce is allowed to use the same early
    # threshold only when at least four independent confirmations exist.
    fast_path_threshold = 58.0 if (fast_path_active and rejection_evidence >= 4) else 62.0
