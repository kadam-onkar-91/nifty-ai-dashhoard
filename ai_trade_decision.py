...
ENTRY_SCORE_MIN = 42.0      # minimum evidence score for CONTINUATION plays (0-100) -- was 46, eased slightly for trade count
REVERSAL_SCORE_MIN = 40.0   # minimum evidence score for BOUNCE/REJECTION plays (they need level + candle both) -- was 44
...
def generate_trade_decision(live_price, level_prediction, atr, max_pain=None,
    ...
    for c in ranked:
        ...
        if c["pa"] < 12 or c["loc"] < (12 if is_rev else 6):
            ...
            continue
        # post-close pause: only for REVERSAL plays (bounce/rejection). A
        # genuine trend continuation (breakdown/breakout) is exactly the
        # "market abhi bhi girta/chadhta ja raha hai" move we should NOT
        # sit out for 3 minutes -- so continuation plays skip this gap.
        if is_rev and _post_close_cooldown_active():
            blocked_reason = blocked_reason or (
                f"{_summary(c)}: pichla trade abhi-abhi close hua - reversal re-entry ke liye "
                f"{POST_CLOSE_COOLDOWN_MIN} min ka gap (continuation trades is gap ko skip karte hain)")
            continue
