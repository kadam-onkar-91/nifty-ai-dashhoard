...
ENTRY_SCORE_MIN = 46.0      # minimum evidence score for CONTINUATION plays (0-100) -- back to 46: continuation
...
REVERSAL_SCORE_MIN = 40.0   # minimum evidence score for BOUNCE/REJECTION plays -- kept eased since these
                             # already hard-require a genuine confirmed rejection pattern before scoring at all
MIN_CONFIDENCE = 54.0       # eased slightly from 58 -- 58 was blocking almost every setup, and the
...
MIN_FACTORS_TRUE = 11       # eased from 15 -- 15/30 was too strict (near-zero trades some days). This is
...
REQUIRE_OI_ALIGNED = True   # HARD-ish gate: OI must not be heavily against the trade (that part stays
...
POST_CLOSE_COOLDOWN_MIN = 3 # minutes to pause ANY new entry right after ANY trade closes (WIN/LOSS/EXPIRED)
NEAR_ZONE_ATR_FRAC = 0.4    # "same zone as the last trade" = within this many ATRs of its entry
NEAR_ZONE_SCORE_BUMP = 6.0  # extra score required to repeat the SAME side from that same zone
...
DROUGHT_MAX_STEPS = 3           # hard cap -- never eases past this many steps
...
def _drought_steps():
    """How many easing steps are active right now, based on minutes since the
    last setup EVER logged (OPEN/WIN/LOSS/EXPIRED all count -- any of them
    proves the engine is still actually able to fire). Returns 0 (no easing)
    if there's no history yet, or the DB read fails -- never eases blind."""
    try:
    ...
    try:
        last = datetime.strptime(str(ts), "%Y-%m-%d %H:%M:%S")
    except Exception:
        return 0
    idle_min = (datetime.now() - last).total_seconds() / 60.0
...
def _score_candidate(play, d, X):
    ...
    if reversal:
        ...
        for lv in levels:
            ...
            if lv.get("side") != want:
                continue
            if d == 1:
                inside = (live - 0.7 * atr) <= lv["price"] <= live
            else:
                inside = live <= lv["price"] <= (live + 0.7 * atr)
            if inside:
    ...
    if play == "BOUNCE_BUY":
        ...
        _genuine_rejection = bool(conf.get("support_rejection_confirmed"))
        ...
        _genuine_rejection = bool(conf.get("resistance_rejection_confirmed"))
        _strong_oi_wall = bool(lvl and _is_oi_level(lvl) and _oi_wall_magnitude(X["level_text"](lvl["price"]))[1] >= 1.0)
        if _strong_incoming and not _genuine_rejection and not _strong_oi_wall:
            pa -= 16
        ...
        if lvl is not None:
            ...
            fake_breakout = {"flags": fb_flags, "count": fb_count, "suspected": fb_count >= 2}
            if fb_count >= 3:
                misses.append(f"Yeh FAKE BREAKOUT / trap lag raha hai ({fb_count}/6 warning signs: "
                              f"{', '.join(k for k, v in fb_flags.items() if v)}) -- genuine break/momentum confirm "
                              "nahi ho raha, entry skip kar raha hai taaki OI/S-R ka fake break chase na ho.")
                return None
            elif fb_count == 2:
...
def _build_trade_levels(cand, X):
    ...
    if d == 1:
        by_dist = sorted([lv for lv in levels if lv["price"] > live + 0.01], key=lambda x: x["price"])
    else:
        by_dist = sorted([lv for lv in levels if lv["price"] < live - 0.01], key=lambda x: -x["price"])
    ...
    if picked:
        ...
        if nearest_wall_dist is not None and nearest_wall_dist < fallback_dist:
...
def generate_trade_decision(live_price, level_prediction, atr, max_pain=None,
    ...
    for c in ranked:
        ...
        base_need = REVERSAL_SCORE_MIN if is_rev else ENTRY_SCORE_MIN
        ...
        if _cooldown_active(direction_name):
            ...
            continue
        zone_bump = _recent_zone_score_bump(direction_name, X["live"], X["atr"])
        if zone_bump and c["score"] < base_need + zone_bump:
            blocked_reason = blocked_reason or (
                f"{_summary(c)}: last trade was almost this same level -- proven zone, "
                f"needs {base_need + zone_bump:.0f}+ this time (has {c['score']:.0f}) in case it reverses instead")
            continue
    ...
    eff_min_factors = max(DROUGHT_FACTORS_FLOOR, MIN_FACTORS_TRUE - DROUGHT_FACTORS_STEP * drought_steps)
    ...
    if REQUIRE_OI_ALIGNED and not factor_flags.get("oi_aligned") and not level_well_proven:
        return _no(f"{_summary(chosen)} mila (score {chosen['score']:.0f}), lekin option-chain OI heavy support "
                   f"nahi dikha raha AUR yeh level sirf ek confirmation se bana hai (strength "
                   f"{(lvl.get('strength', 0) if lvl else 0):.1f}) -- itna proven nahi, skip.",
                   market_view=market_view, factors_true=factors_true, factors_total=factors_total,
                   factor_flags=factor_flags)
    if factors_true < eff_min_factors:
        return _no(f"{_summary(chosen)} mila (score {chosen['score']:.0f}), lekin sirf {factors_true}/{factors_total} "
                   f"factors align ho rahe hain (< {eff_min_factors} chahiye) -- pura tool ka data isse zyada "
                   f"agree nahi kar raha, is liye skip.{drought_note}",
                   market_view=market_view, factors_true=factors_true, factors_total=factors_total,
                   factor_flags=factor_flags)
    ...
    if confidence_pct < eff_min_confidence:
        return _no(f"{_summary(chosen)} mila, lekin apne resolved trades se seekha hua confidence {confidence_pct:.1f}% "
                   f"(< {eff_min_confidence:.0f}%) hai - engine is pattern ko abhi skip kar raha hai.{drought_note}",
                   market_view=market_view, factors_true=factors_true, factors_total=factors_total,
                   factor_flags=factor_flags)
