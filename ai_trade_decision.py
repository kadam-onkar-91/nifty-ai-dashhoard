...
REQUIRE_OI_ALIGNED = True   # HARD gate: option-chain OI/PCR must confirm the direction (heavy put OI under a
                             # BUY, heavy call OI over a SELL) before a trade is allowed at all. OI was the
                             # single most-requested factor to actually enforce, not just score.
...
def generate_trade_decision(live_price, level_prediction, atr, max_pain=None,
    ...
    if REQUIRE_OI_ALIGNED and not factor_flags.get("oi_aligned"):
        return _no(f"{_summary(chosen)} mila (score {chosen['score']:.0f}), lekin option-chain OI/PCR is "
                   f"direction ko confirm nahi kar raha -- OI ke bina trade nahi lega.",
