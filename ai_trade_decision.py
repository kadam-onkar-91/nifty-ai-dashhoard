...
def generate_trade_decision(live_price, level_prediction, atr, max_pain=None,
    ...
    if isinstance(sr_context, dict):
        ...
        if loc in ('AT_RESISTANCE', 'NEAR_RESISTANCE') and direction == 'BUY' and not conf.get('breakout_confirmed'):
            return {"has_setup": False, "reason": "Price is at/near resistance, but a confirmed breakout is not present yet — BUY blocked. Wait for close above the resistance zone with confirmation.", "context_audit": context_audit}
        if loc in ('AT_SUPPORT', 'NEAR_SUPPORT') and direction == 'SELL' and not conf.get('breakdown_confirmed'):
            return {"has_setup": False, "reason": "Price is at/near support, but a confirmed breakdown is not present yet — SELL blocked. Wait for close below the support zone with confirmation.", "context_audit": context_audit}
        if loc == 'MIDRANGE':
            return {"has_setup": False, "reason": "Price is in the middle of the current support/resistance range — location is poor, so the engine will not force a trade.", "context_audit": context_audit}
    ...
    if lp is None:
        return {"has_setup": False, "reason": "No key level (from the single-nearest predictor OR the "
                                               "full ladder) is close enough with a matching directional read right now.",
                "context_audit": context_audit}
