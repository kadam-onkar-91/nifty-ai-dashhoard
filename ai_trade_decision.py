...
def generate_trade_decision(live_price, level_prediction, atr, max_pain=None,
    ...
    if isinstance(sr_context, dict):
        ...
        _early_entry_ok = (_early_level_pct >= 70.0 and _early_checks >= 4)
    ...
    if critical_true < 4 or hard_conflicts >= 1 or unknown_major >= 2 or not factor_flags.get('no_opposite_transition') or (not factor_flags.get('entry_timing_safe') and not _early_entry_ok) or (reversal_used and ((direction == "SELL" and _bearish_reversal_score < 5) or (direction == "BUY" and _bullish_reversal_score < 5))):
        return {"has_setup": False,
    ...
    _entry_floor = 62.0 if _early_entry_ok else 66.0
