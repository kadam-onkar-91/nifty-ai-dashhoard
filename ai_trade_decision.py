...
def _score_candidate(play, d, X):
    ...
    if reversal:
        ...
        if _is_oi_level(lvl):
            loc_pts = min(28.0, loc_pts + 3.0)
            reasons.append(f"{want.capitalize()} {lvl['price']:,.1f} -- heavy Option-chain OI writer wall "
                          f"({lvl['src']}, strength {lvl['strength']:.1f}) -- iss tarah ke level se bounce/"
        ...
        if broken:
            ...
            if _is_oi_level(lvl):
                ...
                loc_pts = min(26.0, loc_pts + 4.0)
                reasons.append("yeh ek heavy Option-chain OI wall tha jo abhi toota -- writers ka stop-hunt/unwind, "
                              "normal level-break se zyada strong continuation signal")
        elif at_low:
            loc_pts = 10.0
        ...
        if _wall:
            if _is_oi_level(_wall):
                ...
                misses.append(f"Support {_wall['price']:,.1f} abhi tak unbroken hai AUR yahan heavy Option-chain OI "
                              "hai (writers actively defending) -- yeh khatarnak hai, isme seedha ghuskar breakdown "
        ...
        if _wall:
            if _is_oi_level(_wall):
                ...
                misses.append(f"Resistance {_wall['price']:,.1f} abhi tak unbroken hai AUR yahan heavy Option-chain OI "
                              "hai (writers actively defending) -- yeh khatarnak hai, isme seedha ghuskar breakout "
    ...
    if (d == 1 and "heavy put oi" in ftxt) or (d == -1 and "heavy call oi" in ftxt):
        ex += 6; reasons.append("heavy OI wall at this level supports the level (put/call writers defending it)")
...
def generate_trade_decision(live_price, level_prediction, atr, max_pain=None,
    ...
    if REQUIRE_OI_ALIGNED and not factor_flags.get("oi_aligned") and not level_well_proven:
        return _no(f"{_summary(chosen)} mila (score {chosen['score']:.0f}), lekin option-chain OI heavy support "
                   ...
                   market_view=market_view, factors_true=factors_true, factors_total=factors_total,
                   factor_flags=factor_flags)
    if factors_true < MIN_FACTORS_TRUE:
        return _no(f"{_summary(chosen)} mila (score {chosen['score']:.0f}), lekin sirf {factors_true}/{factors_total} "
                   f"factors align ho rahe hain (< {MIN_FACTORS_TRUE} chahiye) -- pura tool ka data isse zyada "
                   f"agree nahi kar raha, is liye skip.",
                   market_view=market_view, factors_true=factors_true, factors_total=factors_total,
                   factor_flags=factor_flags)
