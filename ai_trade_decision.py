...
def generate_trade_decision(live_price, level_prediction, atr, max_pain=None,
    ...
    if is_choppy and not early_trigger:
        return {"has_setup": False, "reason": "Market Structure is Choppy/Range-bound right now -- waiting for a cleaner structural setup.",
                "context_audit": context_audit}

    effective_signal = signal_code if signal_code in (1, -1) else (early_dir if early_trigger else 0)
    if effective_signal == 0:
        return {"has_setup": False, "reason": "No sufficiently strong directional setup yet; waiting for main or early level signal.", "context_audit": context_audit}
    ...
    if isinstance(sr_context, dict):
        ...
        if direction == 'BUY' and bool(timing.get('buy_adverse_move_risk')) and not _early_entry_ok:
            return {"has_setup": False,
                    "reason": "BUY blocked: immediate entry-timing risk is high and the early evidence is not strong enough.",
                    "context_audit": context_audit}
        if direction == 'SELL' and bool(timing.get('sell_adverse_move_risk')) and not _early_entry_ok:
            return {"has_setup": False,
                    "reason": "SELL blocked: immediate entry-timing risk is high and the early evidence is not strong enough.",
                    "context_audit": context_audit}

        # EARLY VALIDATED ENTRY: allow a strong move to trigger before a
        # second retest, but only when several independent live factors agree.
        ...
        _early_entry_ok = (_early_level_pct >= 65.0 and _early_checks >= 3)
        ...
        if at_resistance and direction == 'BUY':
            if not conf.get('breakout_retest_confirmed') and not _early_entry_ok:
        ...
        if at_support and direction == 'SELL':
            if not conf.get('breakdown_retest_confirmed') and not _early_entry_ok:
        ...
        if at_support and direction == 'BUY' and not conf.get('support_rejection_confirmed') and not _early_entry_ok:
        ...
        if at_resistance and direction == 'SELL' and not conf.get('resistance_rejection_confirmed') and not _early_entry_ok:
    ...
    if critical_true < 4 or hard_conflicts >= 2 or unknown_major >= 2 or not factor_flags.get('no_opposite_transition') or (not factor_flags.get('entry_timing_safe') and not _early_entry_ok) or (reversal_used and ((direction == "SELL" and _bearish_reversal_score < 5) or (direction == "BUY" and _bullish_reversal_score < 5))):
        return {"has_setup": False,
    ...
    max_entry_distance = max((0.50 if not breakout_entry else 0.65) * float(atr), 6.0 if not breakout_entry else 8.0)
    ...
    _entry_floor = 60.0 if _early_entry_ok else 64.0
    _critical_floor = 4 if _early_entry_ok else 5
    if confidence_pct < _entry_floor or (confidence_pct < 70.0 and critical_true < _critical_floor):
        return {"has_setup": False,
    ...
    return {
        ...
        "context_audit": context_audit,
        "score_model": "weighted_confluence_v12_balanced_entry",
