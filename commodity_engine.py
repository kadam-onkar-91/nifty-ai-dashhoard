...
def _download_commodity(asset, period="10d", interval="5m"):
    # Free primary feed: XAUS XAU/XAG spot + recorded intraday observations.
    intraday = _xaus_intraday(asset, hours=48)
    if intraday is not None and len(intraday) >= 20:
        return intraday, "XAUS_FREE_SPOT_INTRADAY"
    # Secondary fallback: Yahoo Finance spot symbol.
...
def _download_commodity_long(asset, period="60d", interval="1h"):
    """Supplementary longer-history feed used ONLY for the 4H/1D rows of the
    multi-timeframe table. The free XAUS intraday feed is hard-capped at 48
    hours (see _xaus_intraday), which structurally can never contain the 20+
    bars a 4H/1D resample needs -- those rows would always read UNAVAILABLE
    no matter how the rest of the pipeline is configured. Yahoo Finance's
    hourly series comfortably covers 60 days (well within its ~730-day cap
    for 1h data), which is real, live-updating exchange data -- not
    fabricated -- and is used for nothing except the higher-timeframe trend
    read."""
    symbol = COMMODITIES[asset]["spot_symbol"]
    df = _download(symbol, period=period, interval=interval)
    if df is not None and not df.empty:
...
def _macro_snapshot():
    ...
    gold = {"status":"AVAILABLE", "last":gold_snap["price"], "source":gold_snap["source"]} if gold_snap else _last_change(COMMODITIES["GOLD"]["spot_symbol"], period="10d", interval="1d")
    silver = {"status":"AVAILABLE", "last":silver_snap["price"], "source":silver_snap["source"]} if silver_snap else _last_change(COMMODITIES["SILVER"]["spot_symbol"], period="10d", interval="1d")
...
def analyze_commodity(asset, period="10d", interval="5m", access_token=None):
    ...
    market_source = f"GLOBAL_SPOT {asset == 'GOLD' and 'XAU/USD' or 'XAG/USD'}"
    source_meta = {
        "market": "GLOBAL_SPOT",
