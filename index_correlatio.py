...
def _get_upstox_prev_close(access_token, instrument_key):
    ...
    (day interval) -- not Yahoo. Cached for hours since it can't change
    intraday."""
    if not access_token:
        return None
    try:
...
def get_nifty_change_pct(access_token, live_price):
    """Nifty 50's own day change %, used only for the Bank Nifty divergence
    comparison below. Tries Upstox's own previous-close first, Yahoo only
    as a last resort."""
