...
# =====================================================================
# PHASE 2 — Sector Breadth. Real, static NSE sector classification for
# all 49 constituents above (sector membership doesn't change day to day,
# so this is accurate without needing a live classification feed). Used
# to compute which SECTORS are actually driving the index move, not just
# the index-level advance/decline count.
# =====================================================================
SECTOR_MAP = {
    "HDFCBANK": "Banking", "ICICIBANK": "Banking", "KOTAKBANK": "Banking",
    "AXISBANK": "Banking", "SBIN": "Banking", "INDUSINDBK": "Banking",
    "BAJFINANCE": "Financial Services (NBFC/Insurance)", "BAJAJFINSV": "Financial Services (NBFC/Insurance)",
    "SBILIFE": "Financial Services (NBFC/Insurance)", "HDFCLIFE": "Financial Services (NBFC/Insurance)",
    "SHRIRAMFIN": "Financial Services (NBFC/Insurance)",
    "INFY": "IT", "TCS": "IT", "HCLTECH": "IT", "WIPRO": "IT", "TECHM": "IT", "LTIM": "IT",
    "RELIANCE": "Energy / Oil & Gas", "ONGC": "Energy / Oil & Gas", "COALINDIA": "Energy / Oil & Gas",
    "BHARTIARTL": "Telecom",
    "ITC": "FMCG", "HINDUNILVR": "FMCG", "NESTLEIND": "FMCG", "BRITANNIA": "FMCG", "TATACONSUM": "FMCG",
    "LT": "Infra / Capital Goods / Power", "ADANIPORTS": "Infra / Capital Goods / Power",
    "POWERGRID": "Infra / Capital Goods / Power", "NTPC": "Infra / Capital Goods / Power",
    "ADANIENT": "Infra / Capital Goods / Power",
    ...
    "ULTRACEMCO": "Cement / Materials", "GRASIM": "Cement / Materials",
    "JSWSTEEL": "Metals", "TATASTEEL": "Metals", "HINDALCO": "Metals",
    "ASIANPAINT": "Consumer Durables / Retail", "TITAN": "Consumer Durables / Retail",
    "TRENT": "Consumer Durables / Retail",
}


def get_sector_breadth(df_full):
    """
    Groups the already-fetched full-50 breadth dataframe (Symbol, Change (%))
    by real sector and returns per-sector advances/declines/avg change --
    sorted strongest-to-weakest. Real aggregation of real per-stock data,
    no separate live fetch needed.
    """
    ...
    work = work.dropna(subset=["Sector"])
    if work.empty:
        return None

    rows = []
    for sector, grp in work.groupby("Sector"):
        advances = int((grp["Change (%)"] > 0.05).sum())
        declines = int((grp["Change (%)"] < -0.05).sum())
        rows.append({
            "Sector": sector,
            "Stocks": len(grp),
            "Advances": advances,
            "Declines": declines,
            "Avg Change (%)": round(float(grp["Change (%)"].mean()), 2),
        })

    df_sector = pd.DataFrame(rows).sort_values("Avg Change (%)", ascending=False).reset_index(drop=True)
    return df_sector
