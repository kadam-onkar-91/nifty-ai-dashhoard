...
def _load_upstox_nse_equity_map():
    ...
    try:
        ...
        if 'instrument_type' not in df.columns or 'tradingsymbol' not in df.columns:
            return None, f"Instrument master downloaded but columns unexpected: {list(df.columns)}"
        df = df[df['instrument_type'] == 'EQ']
        if df.empty:
            return None, "Instrument master downloaded but 0 rows matched instrument_type == 'EQ'."
        return dict(zip(df['tradingsymbol'], df['instrument_key'])), f"OK ({len(df)} EQ symbols loaded)"
    except Exception as e:
