...
def _load_upstox_nse_equity_map():
    ...
    (risky -- one typo would silently pull the WRONG stock's data). If this
    download/parse fails for any reason, returns None and the caller falls
    back to Yahoo Finance -- never silently mismatched data.
    """
    try:
...
def _fetch_full50_upstox(access_token):
    """Batched live Upstox quotes for all 50 Nifty constituents, using the
    real instrument master (no guessed/typo-risk ISINs). Returns a list of
    row dicts or None if anything is incomplete/fails."""
    if not access_token:
        return None
    symbol_map = _load_upstox_nse_equity_map()
    if not symbol_map:
        return None

    base_symbols = [s.replace(".NS", "") for s in NIFTY50_YF]
    keys = [symbol_map[s] for s in base_symbols if s in symbol_map]
    if len(keys) < 40:  # too many missing -- don't trust a partial/mismatched set
        return None
...
def get_full_nifty50_breadth(access_token=None):
    ...
    heavyweights proxy above). Tries live Upstox quotes first (100% accurate,
    real-time -- needs login), then falls back to Yahoo Finance (delayed
    ~15-20 min) if Upstox isn't available or the instrument lookup fails.
    Returns: df_full, advances, declines, ratio, status_text
    """
    rows = _fetch_full50_upstox(access_token)
    source = "Upstox (Live)"
    ...
    if rows is None:
        return None, None, None, None, "DATA UNAVAILABLE (both Upstox and Yahoo Finance failed to return enough Nifty 50 data)"
