...
def _fetch_real_heavyweights(access_token):
    ...
    try:
        ...
        prev_close_ref = _get_prev_close_reference_map()
        ...
        for h in HEAVYWEIGHTS:
            ...
            prev_close = prev_close_ref.get(h["symbol"])
            if ltp is None or prev_close is None or prev_close == 0:
                missing.append(f"{h['symbol']} (missing ltp/prev_close_ref)")
                continue
    ...
    try:
        ...
        for i in range(0, len(keys), chunk_size):
            ...
            for match in data.values():
                ...
                ltp = match.get("last_price")
                prev_close = prev_close_ref.get(symbol)
