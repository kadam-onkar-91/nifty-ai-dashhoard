...
def get_nifty_internal_breadth(access_token=None):
    ...
    # Treat tiny/zero moves as flat, not a decline. The old "<= 0 is a
    # decline" rule meant that on a day with no real price movement (e.g.
    # markets closed, or Upstox returning ltp == prev_close for every
    # stock) EVERY stock got counted as declining -- shown as "0
    # Advancing / 12 Declining" even though nothing actually fell. The
    # full-50 breadth below already used this same 0.05% tolerance +
    # flat bucket; this just brings the 12-stock table in line with it.
    TOL = 0.05
    advances = int((df_heavyweights["Change (%)"] > TOL).sum())
    declines = int((df_heavyweights["Change (%)"] < -TOL).sum())
    unchanged = len(df_heavyweights) - advances - declines
    ...
    breadth_status = (f"{label} ({advances} up, {declines} down, {unchanged} flat of "
                      f"{len(HEAVYWEIGHTS)}) -- Source: {source}")
