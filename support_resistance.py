...
def predict_level_reaction(df, live_price, atr, fvg_list=None, ob_list=None, trend_bias=0.0, df_option_chain=None, sr_context=None):
    """
    ...
    # When the comprehensive location-first S/R map is available, it is the
    # single source of truth for the early-warning predictor as well. This
    # prevents two different S/R engines from disagreeing (e.g. the dashboard
    # says resistance is 23465 while Early Warning says N/A). The legacy
    # swing/PD levels remain the fallback when no comprehensive map is passed.
    if isinstance(sr_context, dict):
        _ms = sr_context.get('major_support') or sr_context.get('nearest_support')
        _mr = sr_context.get('major_resistance') or sr_context.get('nearest_resistance')
        nearest_support = _safe_float(_ms.get('price') if isinstance(_ms, dict) else _ms)
        nearest_resistance = _safe_float(_mr.get('price') if isinstance(_mr, dict) else _mr)
    else:
...
def build_comprehensive_sr_context(df, live_price, atr=None, df_option_chain=None,
    ...
    for v, src, typ in _daily_weekly_monthly_levels(df):
        base_w = 1.7 if src.startswith('PD') else (1.55 if src.startswith('PW') else 1.45)
        add(v, src, typ, base_w)

    # 3) Today's session and opening range.
    ...
    out['confirmation'] = {
        ...
        'confirmation_rule': 'closed candle + volume expansion OR strong candle body; wick-only breaks rejected'
    }
