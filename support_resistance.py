...
def _adaptive_tolerance(df, atr):
    ...
    # Keep the zone tight enough for NIFTY intraday structure but adaptive to volatility.
    return max(SR_DEFAULT_TICK * 4, min(atr_v * 0.20, 8.0))
...
def build_comprehensive_sr_context(df, live_price, atr=None, df_option_chain=None, fvg_list=None, ob_list=None, liquidity_map=None, pivots=None):
    """Build a location-first, multi-source support/resistance map.

    Sources: confirmed MTF swings, PDH/PDL/PWH/PWL/PDC/PWC/PMH/PML/PMC,
    session high/low, opening range, CPR/pivots, psychological levels, VWAP,
    volume-profile levels, SMC FVG/OB, liquidity/equal highs/lows and option OI.
    Nearby evidence is clustered into zones; each zone keeps source evidence,
    touches and a strength score. Missing sources remain explicitly unavailable.
    """
    out = {'status':'INSUFFICIENT_DATA','live_price':live_price,'zones':[],
           'nearest_support':None,'nearest_resistance':None,'location':'UNKNOWN',
           'option_chain':{'status':'UNAVAILABLE'},'sources':{}}
    price = _safe_float(live_price); atr_v = _safe_float(atr) or 15.0
    ...
    def add(v, source, typ='neutral', weight=1.0):
        ...
        if v is not None and v > 0:
            candidates.append({'price':v,'source':source,'type':typ,'weight':weight})
    # Confirmed multi-timeframe swings.
    for rule, mult, w in [(5,'LTF',1.0),(10,'MTF',1.3),(20,'HTF',1.6)]:
        sh, sl = _swing_points_confirmed(df, rule)
        for v in sh: add(v, f'{mult} Swing High', 'resistance', w)
        for v in sl: add(v, f'{mult} Swing Low', 'support', w)
    for v, src, typ in _daily_weekly_monthly_levels(df): add(v, src, typ, 1.5 if src.startswith('PD') else 1.3)
    for v, src, typ in _today_session_levels(df): add(v, src, typ, 1.2)
    for v, src, typ in _opening_range_levels(df): add(v, src, typ, 1.3)
    if pivots and pivots.get('status') == 'OK':
        for k, typ in [('pivot','neutral'),('tc','resistance'),('bc','support'),('r1','resistance'),('r2','resistance'),('r3','resistance'),('s1','support'),('s2','support'),('s3','support')]:
            add(pivots.get(k), f'CPR/Pivot {k.upper()}', typ, 1.2)
    for v, src, typ in _psych_levels(price): add(v, src, typ, 0.7)
    if 'VWAP' in df.columns:
        v = _safe_float(df['VWAP'].iloc[-1]); add(v, 'Session VWAP', 'neutral', 1.1)
    for v, src, typ in _extract_volume_profile_levels(df, price, atr_v): add(v, src, typ, 1.2)
    for z in (fvg_list or []):
        p = _parse_price(z.get('Price')) if isinstance(z, dict) else None
        if p is not None: add(p, 'SMC FVG', 'support' if 'bull' in str(z.get('Type','')).lower() else 'resistance', 1.2)
    for z in (ob_list or []):
        p = _parse_price(z.get('Price')) if isinstance(z, dict) else None
        if p is not None: add(p, 'SMC Order Block', 'support' if 'bull' in str(z.get('Type','')).lower() else 'resistance', 1.4)
    if liquidity_map:
        for x in liquidity_map.get('levels',[]): add(x.get('price'), x.get('name','Liquidity'), x.get('type','neutral'), 1.3)
        for x in liquidity_map.get('equal_highs',[]): add(x.get('level'), 'Equal High Liquidity', 'resistance', 1.5)
        for x in liquidity_map.get('equal_lows',[]): add(x.get('level'), 'Equal Low Liquidity', 'support', 1.5)
    opt_levels, opt_info = _option_chain_levels(df_option_chain, price, atr_v)
    for v, src, typ in opt_levels: add(v, src, typ, 1.1)
    ...
    for center in clusters:
        ...
        below = center < price
        above = center > price
        # Current price decides the actionable side; source labels remain visible.
        side = 'support' if below else ('resistance' if above else 'current')
        score=sum(x['weight'] for x in members)
        sources=sorted(set(x['source'] for x in members))
        touches=_zone_reactions(df, lo, hi)
        score += min(touches, 8)*0.35
        if len(sources)>=3: score += 1.0
        strength = 'STRONG' if score>=6 else ('MODERATE' if score>=3.5 else 'WEAK')
        zones.append({'price':round(center,2),'low':round(lo,2),'high':round(hi,2),'side':side,
                      'distance_pts':round(abs(price-center),2),'touches':touches,'strength':strength,
    ...
    out['nearest_support']=supports[0] if supports else None
    out['nearest_resistance']=resistances[0] if resistances else None
    ns, nr = out['nearest_support'], out['nearest_resistance']
    if ns and price <= ns['high'] + tol:
        out['location']='AT_SUPPORT'
    elif nr and price >= nr['low'] - tol:
        out['location']='AT_RESISTANCE'
    elif ns and nr and ns['high'] < price < nr['low']:
        # Normalize where price sits inside the current range.
        span=max(nr['low']-ns['high'], 0.01)
        rel=(price-ns['high'])/span
        out['location']='MIDRANGE' if 0.25 <= rel <= 0.75 else ('NEAR_SUPPORT' if rel<0.5 else 'NEAR_RESISTANCE')
    else:
        out['location']='BETWEEN_LEVELS'
    # Confirmation is deliberately strict and uses only completed candles.
    recent=df.tail(3)
    last=recent.iloc[-1]
    prev=recent.iloc[:-1]
    close=_safe_float(last.get('Close')); high=_safe_float(last.get('High')); low=_safe_float(last.get('Low'))
    vol_ok=False
    if 'Volume' in df.columns and len(df)>=10:
        v=pd.to_numeric(df['Volume'],errors='coerce').tail(20)
        vol_ok=bool(_safe_float(v.iloc[-1]) and _safe_float(v.iloc[-1]) > _safe_float(v.iloc[:-1].mean())*1.15)
    bullish_close=close is not None and _safe_float(last.get('Open')) is not None and close>_safe_float(last.get('Open'))
    bearish_close=close is not None and _safe_float(last.get('Open')) is not None and close<_safe_float(last.get('Open'))
    resistance=nr; support=ns
    breakout = bool(resistance and close is not None and close > resistance['high'] and high is not None and high >= resistance['high'] and (vol_ok or bullish_close))
    breakdown = bool(support and close is not None and close < support['low'] and low is not None and low <= support['low'] and (vol_ok or bearish_close))
    support_reject = bool(support and low is not None and low <= support['high'] and close is not None and close > support['high'] and bullish_close)
    resistance_reject = bool(resistance and high is not None and high >= resistance['low'] and close is not None and close < resistance['low'] and bearish_close)
    out['confirmation']={'breakout_confirmed':breakout,'breakdown_confirmed':breakdown,
                         'support_rejection_confirmed':support_reject,'resistance_rejection_confirmed':resistance_reject,
                         'volume_expansion':vol_ok,'latest_close':close}
    out['sources']['swing']='CONFIRMED_SWINGS'
    out['sources']['classic_levels']='PD/PW/PM + session + opening range'
    out['sources']['cpr_pivots']='AVAILABLE' if pivots and pivots.get('status')=='OK' else 'UNAVAILABLE'
    out['sources']['vwap']='AVAILABLE' if 'VWAP' in df.columns else 'UNAVAILABLE'
    out['sources']['volume_profile']='AVAILABLE' if _extract_volume_profile_levels(df,price,atr_v) else 'UNAVAILABLE'
    out['sources']['smc']='FVG/OB INPUTS'
    out['sources']['liquidity']='LIQUIDITY MAP INPUT' if liquidity_map else 'UNAVAILABLE'
    out['sources']['option_chain']=opt_info.get('status','UNAVAILABLE')
    out['status']='OK'
    return out
