...
# Widened from 0.35/0.22: the old values let a zone qualify as "major" only
# ~5-7 pts from price with a ~3-4 pt box, so the nearest support and nearest
# resistance often ended up only 20-30 pts apart -- barely wider than the
# engine's own fixed 20-pt stop-loss. That gave reversal trades almost no
# real room to run before getting stopped by ordinary noise. These are now
# a bit more spread out so a validated zone has to be a real distance from
# price and from the opposite-side zone.
SR_MAJOR_MIN_DISTANCE_ATR = 0.45
...
def build_comprehensive_sr_context(df, live_price, atr=None, df_option_chain=None,
    ...
    # Prefer a zone anchored by the previous completed day's High/Low/Close
    # when one is validated within reach of price. PDH/PDL/PDC already carry
    # the highest structural weight above, but the plain "closest wins" rule
    # could still let a noisier, purely intraday micro-cluster (VWAP/EMA/CPR
    # pivot/opening-range) sit slightly nearer to price and get picked ahead
    # of it. Previous-day levels are what most traders actually react to and
    # are naturally further apart, which also fixes support/resistance
    # sitting unrealistically close together. Only overrides the plain
    # nearest zone when the PD-anchored one is itself actionable and still
    # within a reasonable distance (3 ATR) of price -- otherwise a PD level
    # that gapped far away should not block a genuine nearby reaction.
    def _prefer_pd(cands):
        if not cands:
            return None
        pd_zone = next((z for z in cands if any(
            str(src).startswith(('PDH', 'PDL', 'PDC')) for src in z.get('sources', []))), None)
        if pd_zone and pd_zone['distance_pts'] <= 3.0 * atr_v:
            return pd_zone
        return cands[0]

    ns = _prefer_pd(supports)
    nr = _prefer_pd(resistances)
    ...
    # a separate retest/hold candle: the previous completed candle must have
    # already closed beyond the boundary, while the latest candle retests the
    # old zone and closes back on the accepted side. This prevents the engine
    # from buying the first spike into resistance.
    ...
    if len(df) >= 3 and nr:
        ...
        breakout_retest = bool(
            prev_close is not None and prev_close > nr['high'] and
    ...
    if len(df) >= 3 and ns:
        ...
        breakdown_retest = bool(
            prev_close is not None and prev_close < ns['low'] and
