...
# =====================================================================
# PHASE 2 — Deep Option Chain: ATM±15 window, IV skew, IV percentile
# =====================================================================

def get_atm_window(df_option_chain, spot, n_strikes=15, strike_step=50):
    """Strict ATM±15-strike window for deep option chain analysis (skew,
    OI concentration), separate from the wider NTM_BAND used for the
    main PCR/footprint calc above -- that one intentionally stays wider
    since summing more strikes is fine for a simple OI ratio, but a
    proper skew read should stay tight around the money."""
    if df_option_chain is None or df_option_chain.empty:
        return df_option_chain
    band = n_strikes * strike_step
    window = df_option_chain[(df_option_chain['Strike'] >= spot - band) & (df_option_chain['Strike'] <= spot + band)]
    return window if not window.empty else df_option_chain


def calculate_iv_skew(df_option_chain, spot, strike_step=50):
    """
    25-delta-ish IV skew proxy: compares IV of an OTM strike below spot
    (downside/put-side protection, ~5% OTM) against an OTM strike above
    spot (upside/call-side, ~5% OTM), using the SAME 'IV (%)' column
    already populated from real Upstox Greeks (LIVE) or Black-Scholes on
    real VIX (SIMULATED) -- no separate/fake IV series.

    Positive skew (downside IV > upside IV) is normal/expected for index
    options -- it reflects real-world demand for downside hedges. A
    skew that's unusually flat or inverted is itself informative (complacency
    or aggressive call-side speculation), which is why this is reported as
    a number + a plain-language read, not just a yes/no flag.
    """
    if df_option_chain is None or df_option_chain.empty or 'IV (%)' not in df_option_chain.columns:
        return None

    window = get_atm_window(df_option_chain, spot, n_strikes=15, strike_step=strike_step)
    target_dn = round((spot * 0.95) / strike_step) * strike_step
    target_up = round((spot * 1.05) / strike_step) * strike_step

    def _nearest_iv(target_strike):
        if window.empty:
            return None
        idx = (window['Strike'] - target_strike).abs().idxmin()
        row = window.loc[idx]
        iv = row.get('IV (%)')
        if iv is None or iv == 0:
            return None
        return float(iv), float(row['Strike'])

    dn = _nearest_iv(target_dn)
    up = _nearest_iv(target_up)
    if dn is None or up is None:
        return None

    dn_iv, dn_strike = dn
    up_iv, up_strike = up
    skew = round(dn_iv - up_iv, 2)

    if skew > 1.5:
        read = f"Normal put skew (downside IV {dn_iv}% > upside IV {up_iv}%) -- real demand for downside protection, typical index behaviour"
    elif skew < -1.5:
        read = f"Inverted/call skew (upside IV {up_iv}% > downside IV {dn_iv}%) -- unusual; often reflects aggressive upside speculation or a short-squeeze setup building"
    else:
        read = f"Flat skew (downside IV {dn_iv}% ≈ upside IV {up_iv}%) -- no strong directional hedging bias priced in either wing right now"

    return {
        "downside_strike": dn_strike, "downside_iv": dn_iv,
        "upside_strike": up_strike, "upside_iv": up_iv,
        "skew": skew, "read": read
    }


@st.cache_data(ttl=1800, show_spinner=False)
def _get_vix_history(period="1y"):
    try:
        hist = yf.Ticker("^INDIAVIX").history(period=period)
        if hist.empty:
            return None
...
def get_iv_percentile(current_iv_pct=None):
    """
    Real IV percentile using 1-year of actual India VIX history (India
    VIX IS the market's own real-time implied-vol-of-Nifty-options gauge
    -- it's the standard, honest proxy here since there's no free public
    source of a full historical option-IV surface). Returns None (never
    a fabricated number) if VIX history is unavailable.
    """
    hist = _get_vix_history()
    if hist is None or len(hist) < 30:
        return None
    current = current_iv_pct if current_iv_pct is not None else float(hist.iloc[-1])
    percentile = float((hist < current).sum()) / len(hist) * 100
    percentile = round(percentile, 1)

    if percentile >= 80:
        read = f"IV percentile {percentile}% -- expensive/elevated vs the last year, options are pricing in real fear -- premium SELLING setups favoured"
    elif percentile <= 20:
        read = f"IV percentile {percentile}% -- cheap vs the last year, options are unusually calm -- premium BUYING setups favoured"
    else:
        read = f"IV percentile {percentile}% -- mid-range vs the last year, no strong edge from volatility pricing alone"

    return {"percentile": percentile, "current_iv": round(current, 2), "read": read}
