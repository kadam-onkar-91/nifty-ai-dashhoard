...
## Selected Stock Institutional Dashboard

After Upstox login, the dashboard now shows a **Select Your Stock** box. Enter any
NSE equity trading symbol (for example `RELIANCE`, `TCS`, `HDFCBANK`).

When a stock is selected, the app switches to a stock-isolated pipeline:
- live Upstox equity candles + live LTP + live top-5 equity order-book depth
- stock-specific VWAP/POC, SMC/BOS/CHoCH, liquidity, CPR/pivots and support/resistance
- the same ML walk-forward / confidence-gating architecture
- stock option chain/OI/IV/PCR when Upstox exposes derivatives for that equity
- stock-specific multi-timeframe structure, risk/execution levels and news
- stock-specific fundamentals through Upstox when available
- global macro is contextual only

**NIFTY isolation rule:** stock mode does not substitute NIFTY spot, NIFTY options,
NIFTY futures depth, NIFTY breadth, Bank Nifty correlation, or NIFTY market-wide
signals for missing stock data. If a stock-specific source is unavailable, the
dashboard explicitly shows `DATA UNAVAILABLE` rather than inventing a value.
