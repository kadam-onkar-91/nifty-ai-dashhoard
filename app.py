...
st.markdown("<br>", unsafe_allow_html=True)
st.markdown("---")
st.subheader("📊 Multi-Stock AI Intelligence")
st.caption("Select an NSE equity. The engine uses Upstox Live when available, otherwise explicitly labels Yahoo Finance fallback. Missing fundamentals/news are shown as unavailable — no fake values.")
try:
    _stock_options = multi_stock_engine.NSE_STOCKS
    _watch = st.session_state.setdefault("custom_stock_watchlist", [])
    _search_col, _select_col, _add_col = st.columns([1.3, 1.4, 0.8])
    with _search_col:
        _typed_symbol = st.text_input("🔎 Search NSE symbol", placeholder="RELIANCE / TATASTEEL / HDFCBANK", key="custom_stock_search").strip().upper()
    with _select_col:
        _stock_symbol = st.selectbox("Or choose from universe", _stock_options, index=0, key="multi_stock_symbol")
    _chosen_symbol = _typed_symbol if _typed_symbol else _stock_symbol
    with _add_col:
        if st.button("➕ Add", key="add_custom_stock") and _chosen_symbol:
            import re as _re
            if _re.fullmatch(r"[A-Z0-9&._-]{2,30}", _chosen_symbol):
                if _chosen_symbol not in _watch:
                    _watch.append(_chosen_symbol)
            else:
                st.warning("Symbol format invalid.")
    if _watch:
        st.caption("Custom watchlist: " + ", ".join(_watch))
        _remove = st.selectbox("Remove from custom watchlist", ["—"] + _watch, key="remove_custom_stock")
        if _remove != "—" and st.button("🗑 Remove", key="remove_custom_stock_btn"):
            _watch.remove(_remove); st.rerun()
    _stock_access = st.session_state.get("access_token")
    if st.button("🔎 Analyze Selected Stock", key="analyze_selected_stock"):
        with st.spinner(f"Analyzing {_chosen_symbol}..."):
    ...
    if _sa and _sa.get("symbol") == _chosen_symbol:
        if _sa.get("status") != "OK":
            ...
            c1.metric("LTP", f"₹{_sa['live_price']:,.2f}")
            c2.metric("Trend", _sa["trend"])
            c3.metric("RSI", f"{_sa['rsi']:.1f}")
            c4.metric("Decision", _sa["decision"])
            st.caption(f"Data source: {_sa['data_source']} | Candle: {_sa['timestamp']}")
            t1,t2,t3,t4=st.columns(4)
            t1.metric("Support", f"₹{_sa['support']:,.2f}" if _sa['support'] else "N/A")
            t2.metric("Resistance", f"₹{_sa['resistance']:,.2f}" if _sa['resistance'] else "N/A")
            t3.metric("EMA 20", f"₹{_sa['ema20']:,.2f}")
            t4.metric("EMA 50", f"₹{_sa['ema50']:,.2f}")
            st.markdown("### Fundamental Snapshot")
            _f=_sa['fundamentals']; st.json(_f)
            st.markdown("### NIFTY 50 Fundamental Context")
            _nf=_sa.get('nifty_fundamentals', {})
            if _nf.get('status') == 'AVAILABLE':
                st.caption(f"Source: {_nf.get('source')} | Coverage: {_nf.get('coverage')}/{_nf.get('constituents')} | {_nf.get('method')}")
                st.write({k:v for k,v in _nf.items() if k.endswith('_mean') or k in ('regime_note',)})
            else:
                st.warning("NIFTY 50 fundamentals unavailable from the configured source.")
            st.markdown("### NIFTY 50 News")
            if _sa.get('nifty_news'):
                for _n in _sa['nifty_news']:
                    st.markdown(f"- **{_n['title']}** — {_n['published']}")
            else:
                st.info("NIFTY 50 news unavailable from the current RSS source.")
            st.markdown("### Sector + NIFTY Context")
            st.write({"sector":_sa['sector'],"sector_context":_sa['sector_context'],"nifty_context":_sa['nifty_context']})
            st.markdown("### Latest Company News")
            if _sa['news']:
                for _n in _sa['news']:
                    st.markdown(f"- **{_n['title']}** — {_n['published']}")
            else:
                st.info("Company news unavailable from the current RSS source.")
            st.markdown("### Analysis Summary")
            st.write(f"Technical trend: **{_sa['trend']}**. Momentum: **{_sa['momentum']}**. Bullish score: **{_sa['bullish_score']}**, bearish score: **{_sa['bearish_score']}**. This is a research bias, not a guaranteed trade signal.")
except Exception as _stock_ui_err:
    logger.exception("Multi-stock panel failed")
    st.error(f"Multi-stock analysis error: {_stock_ui_err}")
