...
def _live_dashboard():
    ...
    if df is not None and not df.empty:
        ...
        # Price/indicators above still refresh every 30s as before (that
        # part costs nothing -- Upstox/Yahoo only, no Gemini calls). But
        # calling Gemini itself on every 30s tick was burning through the
        # daily quota in a few hours (2 calls/min x however long the app
        # stays open). The market's overall "read" doesn't genuinely change
        # every 30 seconds anyway, so we now regenerate this AI paragraph
        # at most once every REPORT_REFRESH_SECONDS, and simply re-show the
        # last generated report on the refreshes in between -- cuts Gemini
        # usage for this feature by ~6x with no loss of real-time price data.
        # -------------------------------------------------------------
        REPORT_REFRESH_SECONDS = 180  # regenerate the AI report at most once every 3 minutes

        st.markdown("<br>", unsafe_allow_html=True)
        st.subheader("🦅 Multi-Factor Hybrid AI Deep Reasoning Report")

        now = time.time()
        last_report_time = st.session_state.get("ai_report_last_time", 0)
        needs_new_report = (now - last_report_time) >= REPORT_REFRESH_SECONDS or "ai_report_text" not in st.session_state
        ...
        if needs_new_report:
            with st.spinner("Analyzing Technicals, Option Chain, FII Footprint, Breadth, SMC & Global Macros..."):
                ...
                st.session_state.ai_report_last_time = now
        else:
            ai_report_text = st.session_state.ai_report_text

        seconds_old = int(now - st.session_state.ai_report_last_time)
        st.caption(f"🕒 Report generated {seconds_old}s ago -- refreshes every {REPORT_REFRESH_SECONDS // 60} min to save API quota.")
