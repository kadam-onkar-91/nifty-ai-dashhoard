...
with st.spinner("Fetching real-time feeds & executing quantitative models..."):
    access_token = upstox_auth.get_access_token() if upstox_auth and hasattr(upstox_auth, 'get_access_token') else None

    # 1. Nifty Intraday Data & Technical Indicators
    ...
    # Automatically update pending trade outcomes based on live price
    database.update_pending_outcomes(live_price)
    ...
    df_breadth, total_adv, total_dec, breadth_ratio, breadth_status = market_breadth.get_nifty_internal_breadth(access_token)
...
with tab4:
    ...
    with col_b2:
        st.write(f"**Current Trend Health:** {breadth_status} | **Ratio:** {breadth_ratio}")
...
with tab5:
    ...
    st.markdown("---")
    st.subheader("🗄️ Database Performance & Real Historical Accuracy")
    
    stats = database.get_real_accuracy_stats()
    ...
    if stats["win_rate"] is not None:
        d_col3.metric("Real Historical Win Rate", f"{stats['win_rate']}%", f"{stats['total_resolved']} Resolved")
    else:
        d_col3.metric("Real Historical Win Rate", "Not enough data yet", "0 Resolved")
    d_col4.metric("Overall Win Rate %", f"{win_rate}%")
