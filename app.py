...
def _live_dashboard():
    ...
    if df is not None and not df.empty:
        ...
        # Keep Early Warning and the location-first S/R engine on the SAME
        # structural levels. The first early-warning pass above is retained
        # for backward compatibility, then refreshed here using the validated
        # comprehensive S/R map so the UI and trade engine cannot disagree.
        level_prediction = support_resistance.predict_level_reaction(
            df=df, live_price=live_price, atr=ai_analysis['tech_metrics'].get('atr', 15.0),
            fvg_list=fvg, ob_list=ob, trend_bias=ai_analysis.get('tech_score', 0.0),
            df_option_chain=df_option_chain, sr_context=sr_context
        )
        ai_analysis['level_prediction'] = level_prediction
