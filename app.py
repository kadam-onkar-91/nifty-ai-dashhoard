...
def _live_dashboard():
    ...
    if df is not None and not df.empty:
        ...
        # -------------------------------------------------------------
        # ⚡ NEW: PRE-MOVE / IMMINENT BREAKOUT DETECTOR
        # Fires BEFORE the big directional candle prints -- reads the same
        # BB-width compression that `is_choppy` uses to go silent, but
        # instead reports which way the coil is leaning + the exact
        # trigger prices to watch. See pre_move_detector.py for why this
        # doesn't touch the AI Trade Decision Engine's "no trade in choppy
        # regime" discipline -- it's a heads-up, not a trade call.
        # -------------------------------------------------------------
        pre_move = pre_move_detector.detect_pre_move_squeeze(df, fvg=fvg, ob=ob, candle_pattern=candle_pattern)

        # Edge-triggered Telegram ping -- only once per NEW squeeze+lean
        # combo (not every 30s refresh while the same coil holds), same
        # session_state anti-spam pattern as the confluence alert below.
        if alert_sys is not None and pre_move['squeeze_active'] and pre_move['lean'] in ("BULLISH", "BEARISH"):
            _pm_signature = (pre_move['lean'], pre_move['squeeze_candles'] // 3)
            _pm_last_sig = st.session_state.get('last_pre_move_signature')
            _pm_last_ts = st.session_state.get('last_pre_move_alert_ts', 0)
            _pm_now_ts = time.time()
            if _pm_signature != _pm_last_sig and (_pm_now_ts - _pm_last_ts) >= 300:
                _pm_sent = alert_sys.send_pre_move_alert(
                    lean=pre_move['lean'], price=live_price,
                    squeeze_candles=pre_move['squeeze_candles'],
                    lean_score_pct=pre_move['lean_score_pct'],
                    trigger_up=pre_move['trigger_up'], trigger_down=pre_move['trigger_down'],
                    top_factor=pre_move['factors'][0] if pre_move['factors'] else None
                )
                if _pm_sent:
                    st.session_state['last_pre_move_signature'] = _pm_signature
                    st.session_state['last_pre_move_alert_ts'] = _pm_now_ts
        ...
        # India news sentiment -- moved up here (was previously computed
        # further down, only inside the decision-engine block) so BOTH the
        # new Next-5-Min consolidator AND the existing decision engine can
        # use the same single lookup instead of duplicating it.
        ...
        try:
            ...
            if not _india_row.empty:
                india_news_sentiment = _india_row.iloc[0]["Net Sentiment"]
        except Exception:
            pass

        # -------------------------------------------------------------
        # 🎯 NEW: NEXT 5-MINUTE DIRECTION CALL — single consolidated read
        # Combines EVERY signal this dashboard already computes (main
        # confluence engine, validated ML model, level-approach read,
        # compression lean, sniper VWAP+CPR bias, Bank Nifty divergence,
        # breadth, global markets, VIX, max-pain magnet, India news) into
        # ONE weighted lean, instead of making you read 6 sections and
        # combine them yourself. See next_move_predictor.py for the exact
        # weighting and for why "100% accurate" isn't a claim any tool can
        # honestly make -- this is a probability lean, not a guarantee, and
        # it says so on-screen below.
        # -------------------------------------------------------------
        # -------------------------------------------------------------
        # 🎯 NEW: NEXT 5-MINUTE DIRECTION CALL — single consolidated read
        # Combines EVERY signal this dashboard already computes (main
        # confluence engine, validated ML model, level-approach read,
        # compression lean, sniper VWAP+CPR bias, candle pattern, market
        # structure, Bank Nifty divergence, breadth, global markets, VIX,
        # max-pain magnet, India news) into ONE weighted lean.
        #
        # SMOOTHED across refreshes (st.session_state) with hysteresis so
        # the call doesn't flip UP/DOWN/SIDEWAYS every 30s from small
        # tick-to-tick noise -- it only changes once the lean has genuinely,
        # persistently shifted. See next_move_predictor.smooth_and_classify.
        # -------------------------------------------------------------
        next_move_raw = next_move_predictor.predict_next_5min_direction(
            ai_analysis=ai_analysis, ml_results=ml_results, level_prediction=level_prediction,
            pre_move=pre_move, sniper=sniper, banknifty_correlation_note=banknifty_correlation_note,
            breadth_status=breadth_status, avg_market_change=avg_market_change, live_vix=live_vix,
            max_pain=max_pain, live_price=live_price, is_choppy=is_choppy,
            india_news_sentiment=india_news_sentiment, candle_pattern=candle_pattern, smc_event=smc_event
        )
        _prev_smoothed = st.session_state.get('next_move_smoothed')
        next_move = next_move_predictor.smooth_and_classify(next_move_raw['raw_up_pct'], _prev_smoothed)
        st.session_state['next_move_smoothed'] = next_move['smoothed']
        next_move['reasoning'] = next_move_raw['reasoning']  # for the expander below

        st.subheader("🎯 Next 5-Minute Direction Call")
        if next_move['direction'] == "UP":
            st.success(f"## ⬆️ UP — {next_move['confidence_pct']}% confidence")
        elif next_move['direction'] == "DOWN":
            st.error(f"## ⬇️ DOWN — {next_move['confidence_pct']}% confidence")
        else:
            st.warning(f"## ↔️ SIDEWAYS — {next_move['confidence_pct']}% confidence")
        with st.expander("🔍 Is call ke peeche kaunse factors vote kar rahe hain", expanded=False):
            if next_move['reasoning']:
                for r in next_move['reasoning']:
                    st.write(f"- {r}")
            else:
                st.caption("Abhi koi factor clearly ek taraf vote nahi kar raha.")
        ...
        # -------------------------------------------------------------
        # ⚡ NEW: PRE-MOVE / IMMINENT BREAKOUT DETECTOR — DISPLAY
        # This is the "move aayega ya nahi, aur kis taraf" heads-up --
        # fires while price is still coiled, not after the candle breaks.
        # -------------------------------------------------------------
        st.markdown("<br>", unsafe_allow_html=True)
        st.subheader("⚡ Pre-Move Alert: Compression / Imminent Breakout Detector")
        st.caption("Jab Bollinger Band width ek multi-candle LOW pe compress ho jaati hai, matlab market coil "
                   "kar raha hai — aksar isi ke baad ek bada directional candle aata hai. Ye section us coil ke "
                   "ANDAR hi bata deta hai ki lean kis taraf hai aur breakout confirm hone ke liye kaunsa price "
                   "cross karna padega, taaki move print hone se PEHLE pata chale.")

        if pre_move['squeeze_active']:
            pm1, pm2, pm3 = st.columns(3)
            pm1.metric(label="Coiling Since", value=f"{pre_move['squeeze_candles']} candles")
            pm2.metric(label="Lean", value=pre_move['lean'], delta=f"{pre_move['lean_score_pct']}% score")
            trig_up = f"₹{pre_move['trigger_up']:,.2f}" if pre_move['trigger_up'] else "N/A"
            trig_down = f"₹{pre_move['trigger_down']:,.2f}" if pre_move['trigger_down'] else "N/A"
            pm3.metric(label="Watch Triggers", value=f"↑{trig_up}", delta=f"↓{trig_down}")

            if pre_move['lean'] == "BULLISH":
                st.success(f"**Compression detected, lean is BULLISH.** Upar ₹{pre_move['trigger_up']:,.2f} cross "
                           f"hote hi breakout confirm — tab tak ye sirf ek build-up hai, trade nahi.")
            elif pre_move['lean'] == "BEARISH":
                st.error(f"**Compression detected, lean is BEARISH.** Neeche ₹{pre_move['trigger_down']:,.2f} "
                         f"break hote hi breakdown confirm — tab tak ye sirf ek build-up hai, trade nahi.")
            else:
                st.warning("**Compression detected but lean abhi balanced hai (koi clear taraf nahi).** Dono "
                           "trigger levels pe nazar rakho — jo pehle break ho, wahi direction.")

            with st.expander("🔍 Lean Factors (kyun ye taraf)", expanded=False):
                if pre_move['factors']:
                    for f in pre_move['factors']:
                        st.write(f"- {f}")
                else:
                    st.caption("Abhi koi extra tilting factor detect nahi hua — lean purely price-position se hai.")
        else:
            st.info("Abhi koi genuine compression/coil detect nahi hua — market normal volatility me hai, "
                    "koi imminent-breakout warning nahi.")
        ...
        if _open_setup:
            ...
            _decision = ai_trade_decision.generate_trade_decision(
                ...
                india_news_sentiment=india_news_sentiment, level_ladder=level_ladder,
            ...
            if not _decision['has_setup']:
                ...
                if st.button("📝 Is setup ko log karo (tracking ke liye)"):
                    ...
                    if _sid:
                        st.success(f"Logged (id {_sid}) — ab isse track karega aur outcome se seekhega.")
                    else:
                        st.info("Ye setup already track ho raha hai.")
