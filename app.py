...
# Native auto-refresh (no external package). IMPORTANT: a plain HTML
# meta-refresh reloads the whole browser page, which wipes st.session_state
# on every refresh -- that was silently logging Upstox out every 30s. This
# uses Streamlit's own st.fragment(run_every=...) instead, which reruns the
# app over the SAME session/websocket connection, so session_state (and the
# Upstox login token) survives across refreshes.
