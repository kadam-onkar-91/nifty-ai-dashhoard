...
def _render_stock_dashboard(symbol):
    ...
    with tabs[1]:
        ...
        mtf_rows = []
        for tf, v in tc.get("mtf", {}).items():
            if isinstance(v, dict):
                mtf_rows.append({"Timeframe": tf, "Status": v.get("status"), "Trend": v.get("trend", "N/A"),
                                  "Structure": v.get("structure", "N/A"), "Bars": v.get("bars", "N/A")})
        if mtf_rows: st.dataframe(pd.DataFrame(mtf_rows), hide_index=True, use_container_width=True)
        st.markdown("#### SMC / ICT")
        smc = tc.get("smc", {})
        st.write({"Fair Value Gaps": smc.get("fvg", []), "Order Blocks": smc.get("order_blocks", []), "Liquidity Sweeps": smc.get("liquidity_sweeps", [])})
        st.markdown("#### Market structure + candle")
        st.write({"Market Structure": tc.get("market_structure", []), "Latest Candle": tc.get("candlestick", {})})
        st.markdown("#### Liquidity map")
        liq = tc.get("liquidity", {})
        st.dataframe(pd.DataFrame(liq.get("levels", [])[:12]), hide_index=True, use_container_width=True) if liq.get("levels") else st.info("No liquidity levels available.")
...
try:
    ...
    if _ca and _ca.get("asset") == _commodity_asset:
        if _ca.get("status") != "OK":
            ...
            st.json(_ca["fundamentals"])
            st.markdown("### Macro / Cross-Market Context")
            st.json(_ca["macro_context"])
            st.markdown("### Structural / Liquidity / Regime Context")
            st.json({"SMC/ICT": _ca.get("smc_context"), "Liquidity": _ca.get("liquidity_context"), "Market Regime": _ca.get("market_regime"), "Multi-Timeframe": _ca.get("multi_timeframe")})
            st.markdown("### AI Trade Candidate (requires full AI validation)")
            st.json(_ca.get("trade_candidate", {}))
