...
if df is not None and not df.empty:
    ...
    proximity_info = smart_money.detect_approaching_zone(df, live_price)
    directional_bias = smart_money.calculate_directional_bias(df)
    break_vs_bounce = smart_money.calculate_break_vs_bounce(df, live_price, proximity_info)
    ict_bias = smart_money.calculate_ict_bias(df, live_price, proximity_info)
    ...
    ai_analysis = ai_engine.analyze(
        ...
        break_vs_bounce=break_vs_bounce,
    ...
    # -------------------------------------------------------------
    # ANTICIPATORY LEVEL PROXIMITY + DIRECTIONAL BIAS + BREAK-VS-BOUNCE
    # PROBABILITY — fires BEFORE price touches a support/resistance level,
    # not after. Directly answers: which way is price leaning right now,
    # and if it's at a level, will it break through or bounce -- with %.
    # -------------------------------------------------------------
    st.markdown("<br>", unsafe_allow_html=True)
    st.subheader("🎯 Anticipatory Direction & Break-vs-Bounce Probability")

    dir_label = directional_bias.get("direction", "NEUTRAL").replace("_", " ").title()
    st.info(f"📍 **Current Momentum Lean:** Price is currently leaning **{dir_label}** "
            f"({directional_bias.get('confidence_pct', 50)}% confidence) — this updates continuously, "
            f"before any level is even reached.")

    # ICT (Inner Circle Trader) Premium/Discount + Liquidity Pools
    pd_zone = ict_bias.get("premium_discount", {})
    zone_label = pd_zone.get("zone", "UNKNOWN")
    if zone_label in ("PREMIUM", "DISCOUNT"):
        zone_color = st.error if zone_label == "PREMIUM" else st.success
        zone_color(f"📐 **ICT Zone: {zone_label}** ({pd_zone.get('position_pct')}% of recent range) — "
                   f"{ict_bias.get('description')}")
    else:
        st.caption(f"📐 ICT Zone: {zone_label} — {ict_bias.get('description')}")

    if ict_bias.get("pool_warning"):
        st.warning(ict_bias["pool_warning"])

    prox_status = proximity_info.get("status")
    if prox_status in ("APPROACHING_SUPPORT", "APPROACHING_RESISTANCE") and break_vs_bounce:
        level_type = break_vs_bounce["level_type"]
        level_val = break_vs_bounce["level"]
        break_pct = break_vs_bounce["break_probability_pct"]
        bounce_pct = break_vs_bounce["bounce_probability_pct"]

        bp1, bp2 = st.columns(2)
        bp1.metric(f"🔻 Break-Through Probability", f"{break_pct}%")
        bp2.metric(f"🔺 Bounce/Reject Probability", f"{bounce_pct}%")

        if level_type == "Support":
            lean = "BOUNCE (bullish reaction likely)" if bounce_pct >= break_pct else "BREAK BELOW (bearish continuation likely)"
        else:
            lean = "REJECT (bearish reaction likely)" if bounce_pct >= break_pct else "BREAK ABOVE (bullish continuation likely)"

        st.warning(f"🎯 **Price is at {level_type} ₹{level_val}** — More likely outcome: **{lean}** "
                   f"(tested {break_vs_bounce['touches_tested']}x recently"
                   f"{', RSI ' + str(break_vs_bounce['rsi']) if break_vs_bounce.get('rsi') is not None else ''}). "
                   f"This is a probability estimate from momentum + RSI exhaustion + level test count -- "
                   f"not a guarantee, use it to lean your decision before the level is confirmed broken.")
    elif prox_status in ("ABOVE_RECENT_RANGE", "BELOW_RECENT_RANGE"):
        st.caption("📍 Price has already moved past the recent range boundary — momentum continuation zone, not an approach anymore.")
    else:
        st.caption(f"📍 Mid-range: Support ₹{proximity_info.get('support')} | Resistance ₹{proximity_info.get('resistance')} -- no key level close by right now.")
    ...
    # -------------------------------------------------------------
    # INTERACTIVE AI CHAT — ask free-form questions about everything
    # computed above. Every turn re-sends this run's live numbers, so
    # answers always reflect the current refresh, not a stale one.
    # -------------------------------------------------------------
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("---")
    render_ai_chat(
