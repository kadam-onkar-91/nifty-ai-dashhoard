...
import risk_engine
import multi_stock_engine
import commodity_engine
import global_research
...
# you log in). Leave it empty and you get the Nifty 50 index dashboard
# below (default, unchanged). Type ANY NSE stock symbol here and click
# "Load Stock" and the ENTIRE dashboard for the rest of this page
# switches to that stock: same engines, same weights, same layout, same
# colours/style as the Nifty 50 dashboard -- just built from that
# stock's own real market data instead of Nifty's.
# -------------------------------------------------------------
with st.sidebar:
    st.markdown("### 🔎 Select Your Stock")
    _sidebar_symbol_raw = st.text_input(
        "Select your stocks",
        placeholder="e.g. RELIANCE, TCS, HDFCBANK",
        key="sidebar_stock_input",
        help="Koi bhi NSE stock ka naam/symbol daalo — poora dashboard usi stock ka ban jaayega, ekdum Nifty 50 wale jaisa hi.",
    ).strip().upper()
    ...
    if st.session_state.get("active_symbol"):
        ...
        st.caption("Abhi Nifty 50 dashboard dikh raha hai.")

ACTIVE_SYMBOL = st.session_state.get("active_symbol") or None
...
def _live_dashboard():
    ...
    if df is not None and not df.empty:
        ...
        st.session_state["_last_global_avg_change"] = avg_market_change
        st.session_state["_last_sentiment_score"] = global_sentiment_score
        live_vix = global_markets.get_live_vix(df_global_markets)  # NEW: real VIX, not hardcoded
        # Cached for the "Select Your Stock" dashboard below, so its macro
        # score input is this run's real global sentiment/VIX rather than a
        # placeholder when it hasn't fetched its own copy yet.
        st.session_state["_last_live_vix"] = live_vix
        global_research_snapshot = global_research.build_global_research(
            df_global_markets, df_global_sentiment,
            next((str(r["Net Sentiment"]) for _, r in df_global_sentiment.iterrows() if "India" in str(r.get("Region / Country", ""))), None)
            if df_global_sentiment is not None and not df_global_sentiment.empty else None
        )
        ...
        with st.expander("📉 Order Flow + Real Backtest + Monte Carlo + Drift Detection (Phase 3)", expanded=False):
            ...
            st.markdown("---")
            st.markdown("**Historical ML Walk-Forward Test (upload real NIFTY OHLCV CSV)**")
            st.caption("Ye test historical OHLCV/ML component ko leakage-safe walk-forward se validate karta hai. Full AI trade replay ke liye historical option-chain, order-flow, news aur other dashboard snapshots bhi chahiye — unke bina fake backtest nahi banaya jaata.")
            _hist_file = st.file_uploader("NIFTY historical CSV", type=["csv"], key="historical_ml_backtest_csv")
            if _hist_file is not None:
                try:
                    _hist_df = pd.read_csv(_hist_file)
                    _hist_result = historical_backtest.run_ml_walk_forward(_hist_df)
                    if _hist_result.get("status") == "OK":
                        h1, h2, h3, h4 = st.columns(4)
                        h1.metric("OOS Trades", _hist_result["directional_trades"])
                        h2.metric("OOS Win Rate", f"{_hist_result["win_rate_pct"]}%")
                        h3.metric("Wins / Losses", f"{_hist_result["wins"]} / {_hist_result["losses"]}")
                        h4.metric("Trade Rate", f"{_hist_result["trade_rate_pct"]}%")
                        st.caption(f"OOS predictions: {_hist_result["oos_predictions"]} | Avg confidence: {_hist_result["avg_confidence_pct"]}%")
                        st.write(f"Confusion matrix [-1,0,+1]: {_hist_result["confusion_matrix"]}")
                        st.caption(" | ".join(_hist_result["leakage_controls"]))
                        st.info(_hist_result["note"])
                    else:
                        st.warning(_hist_result.get("note", "Historical test needs more data."))
                except Exception as _e:
                    logger.exception("Historical backtest failed")
                    st.error(f"Historical CSV test failed: {type(_e).__name__}: {_e}")
    ...
    # -------------------------------------------------------------
        ...
        st.session_state['dashboard_context'].update({
            # These two raw price packets are REQUIRED by ai_trade_decision.py.
            # They were accidentally omitted from the context payload, so the
            # decision engine saw an incomplete dashboard on every refresh and
            # returned "Full dashboard context is incomplete" before it could
            # evaluate a single trade.
            "RAW Primary Price/Indicator Data (latest 500 candles)": _ctx_table(df.tail(500)),
            "RAW Latest Price/Indicator Snapshot": _ctx_table(df.tail(1)),
            ...
            "RAW Global Research": str(global_research_snapshot),
            "NIFTY Global Research Bias": str(global_research_snapshot.get("directional_bias", "UNKNOWN")),
        ...
        st.caption("4 playbooks (Support Bounce, Resistance Rejection, Breakdown, Breakout) dono side check karta hai — "
                   "sirf tab bolta hai jab level + candle + baaki factors mil kar score dete hain, har point pe trade nahi. "
        ...
        _decision = None  # set below when the engine runs (stays None while a setup is already open / risk-blocked)
        ...
        if _open_setup:
            ...
            # -------------------------------------------------------------
            # FIX: engine used to evaluate/open a fresh trade on WHATEVER
            # price was last available, even pre-market/after-close/weekend
            # -- so simply reloading the page outside trading hours could
            # fire a "new setup" off a stale/closed-market snapshot. It
            # now only ever opens a NEW trade during real NSE hours
            # (09:15-15:30 IST, Mon-Fri). Existing open positions still
            # get tracked/resolved above regardless of the hour.
            # -------------------------------------------------------------
            st.info(f"⏳ **Market abhi {mkt_status['label']}** — naya trade sirf live NSE hours "
                    "(09:15-15:30 IST, Mon-Fri) mein hi evaluate hota hai, band/pre-market price par nahi.")
        elif data_freshness['is_stale']:
            st.warning("⏳ **Data stale/frozen hai** — fresh live tick milne tak engine koi naya trade evaluate nahi karega.")
        elif risk_state.get('status') != 'OK_TO_TRADE':
            st.warning("🛡️ **Risk Engine blocked a new trade:** " + " | ".join(risk_state.get('block_reasons') or ["risk limit reached"]))
        else:
            ...
            # NIFTY 50-specific news (own RSS, fast) + fundamentals bias (only
            # if the user has already manually refreshed the fundamentals
            # expander above -- that call pulls 50 stocks and is deliberately
            # NOT auto-fetched on every 30s cycle). Both are soft confluence
            # factors folded into the SAME weighted score as everything else,
            # not a separate gate -- see ai_trade_decision.py / trade_learning.py.
            _n50_news_for_ai = st.session_state.get("_nifty50_news_cache")
            if _n50_news_for_ai is None:
                _n50_news_for_ai, _ = multi_stock_engine.get_nifty50_news(limit=10)
            _n50_news_sent = multi_stock_engine.get_nifty50_news_sentiment(_n50_news_for_ai)
            _n50_fund_bias = multi_stock_engine.get_nifty50_fundamentals_bias(
                st.session_state.get("_nifty50_fund_cache"))
            ...
            _decision = ai_trade_decision.generate_trade_decision(
                ...
                global_research=global_research_snapshot,
                nifty50_news_sentiment=_n50_news_sent.get("label"),
                nifty50_fundamentals_bias=_n50_fund_bias.get("label"),
                df=df,
            )
            ...
            _mv = _decision.get('market_view') or {}
            if not _decision['has_setup']:
                ...
                st.write(f"**Underlying Entry:** ₹{_decision['underlying_entry']:,.2f} | "
                        ...
                        + (f" | **Target 2:** ₹{_decision['target2']:,.2f}" if _decision.get('target2') else "")
                        + f" | **R:R** {_decision.get('risk_reward', '-')}")
                st.caption(f"🎯 Playbook: **{_decision.get('playbook_label', '-')}** | Setup score {_decision.get('setup_score', '-')}/100 | "
                          f"Level ₹{_decision['level_price']:,.2f} | {_decision.get('target_note', '')}")
                if _decision.get('reasons'):
                    st.markdown("**Kyun liya:** " + " · ".join(_decision['reasons'][:8]))
                if _decision.get('fake_breakout_suspected'):
                    _fb = _decision.get('fake_breakout_info') or {}
                    st.warning(f"🚧 **Fake-breakout risk detected** ({_fb.get('count', '?')}/6 warning signs) -- "
                              "taken with a heavily penalized score, aur is trade ka outcome self-learning "
                              "factor-reliability training mein COUNT nahi hoga (trap ho sakta hai, genuine "
                              "structural signal nahi).")
                if _decision.get('cautions'):
                    st.caption("⚠️ Dhyan rakho: " + "; ".join(_decision['cautions'][:4]))
                if _decision.get('drought_ease_steps'):
                    st.info(f"🧪 **Learning-mode entry** -- {_decision['drought_ease_steps']} drought-ease step(s) active "
                            "(long quiet gap, so the confluence-count bar was eased a notch; OI-against and "
                            "fake-breakout hard gates were still fully enforced). Treat this one as lower-conviction "
                            "than a normal full-bar setup.")
                ...
                _factor_flags_with_reasoning = dict(_decision['factor_flags'])
                _factor_flags_with_reasoning["_playbook_label"] = _decision.get('playbook_label')
                _factor_flags_with_reasoning["_setup_score"] = _decision.get('setup_score')
                _factor_flags_with_reasoning["_reasons"] = (_decision.get('reasons') or [])[:5]
                # Fake-breakout/fake-move tag: carried into storage so
                # trade_learning.py can exclude this setup's eventual
                # WIN/LOSS from the self-learning factor-reliability stats
                # (a trap resolving as a loss must not teach the engine
                # the underlying factors were bad -- it wasn't a genuine
                # structural failure, it was a fake break).
                _factor_flags_with_reasoning["_fake_breakout_suspected"] = bool(_decision.get('fake_breakout_suspected'))
                _sid = trade_learning.log_setup(
                    ...
                    _decision['confidence_pct'], _factor_flags_with_reasoning
                )
                if _sid:
                    st.success(f"✅ Automatically logged (id {_sid}) — tracking ho raha hai, outcome se seekhega.")

        _mv_show = _decision.get('market_view') if isinstance(_decision, dict) else None
        if _mv_show:
            with st.expander("🧭 Market View — kahan gir sakta hai, kahan se bounce, kahan tak jayega", expanded=True):
                st.markdown(f"**Bias:** {_mv_show.get('bias', '-')}")
                for _ln in _mv_show.get('lines', []):
                    st.markdown(f"- {_ln}")
                _sup_rows = [{"Support": f"₹{x['price']:,.1f}", "Source": x['src'], "Strength": x['strength'], "Door (pts)": x['away_pts']}
                             for x in _mv_show.get('supports', [])]
                _res_rows = [{"Resistance": f"₹{x['price']:,.1f}", "Source": x['src'], "Strength": x['strength'], "Door (pts)": x['away_pts']}
                             for x in _mv_show.get('resistances', [])]
                _mc1, _mc2 = st.columns(2)
                if _sup_rows:
                    _mc1.dataframe(_sup_rows, hide_index=True, use_container_width=True)
                if _res_rows:
                    _mc2.dataframe(_res_rows, hide_index=True, use_container_width=True)
                st.caption("Ye levels engine ke apne 4 playbooks (Support Bounce / Resistance Rejection / Breakdown / Breakout) "
                          "ke liye use hote hain. Guarantee nahi — sirf probability map hai.")
        ...
        with st.expander("📈 Engine ka real track record + factor reliability", expanded=False):
            ...
            if not _recent_setups:
                ...
                _setup_rows = [{
                    ...
                    "Score": s.get('setup_score') if s.get('setup_score') is not None else '-',
                    "Why (kyun liya)": s.get('reasons_short', '-'),
                    "Status": s['status'],
            ...
            _learning_summary = trade_learning.get_learning_summary()
            _rev = _learning_summary.get("immediate_reversal") or {}
            if _rev.get("samples", 0):
                _wr = f"{_rev.get('win_rate')}%" if _rev.get("win_rate") is not None else "Not enough data"
                st.info(f"🔎 Immediate-reversal learning: {_rev.get('samples')} cases | Wins {_rev.get('wins',0)} | Losses {_rev.get('losses',0)} | Win rate {_wr}")
            else:
                st.caption("🔎 Immediate-reversal learning warming up: open trades now record favorable/adverse excursion so repeated entry-after-reversal mistakes can be learned separately.")
            _rel_rows = []
        ...
        st.markdown("<br>", unsafe_allow_html=True)
        with st.expander("🗞️ NIFTY 50 News", expanded=False):
            _refresh_nifty_news = st.button("🔄 Refresh NIFTY 50 News", key="refresh_nifty50_news")
            if _refresh_nifty_news or "_nifty50_news_cache" not in st.session_state:
                _n50_news, _n50_news_status = multi_stock_engine.get_nifty50_news(limit=10)
                st.session_state["_nifty50_news_cache"] = _n50_news
                st.session_state["_nifty50_news_status"] = _n50_news_status
            _n50_news = st.session_state.get("_nifty50_news_cache", [])
            if not _n50_news:
                st.info("NIFTY 50 news unavailable from the current RSS source right now.")
            else:
                for _n in _n50_news[:10]:
                    _title = _n.get("title") or "Untitled"; _pub = _n.get("published") or "Time unavailable"
                    _src = _n.get("source") or "Source unavailable"; _link = _n.get("link")
                    if _link:
                        st.markdown(f"- **{_title}** — {_src} · {_pub} · [Open]({_link})")
                    else:
                        st.markdown(f"- **{_title}** — {_src} · {_pub}")

        st.markdown("<br>", unsafe_allow_html=True)
        with st.expander("📊 NIFTY 50 Fundamentals (constituent aggregate)", expanded=False):
            st.caption("Real NIFTY 50 nahi ek single company hai, isliye ek official PE/EPS nahi hota — ye "
                       "asli 50 constituents ke fundamentals ka equal-weight average hai, honestly labelled "
                       "as an aggregate (Nifty ke official index-weighted number se alag).")
            _refresh_nifty_fund = st.button("🔄 Refresh NIFTY 50 Fundamentals (slow — ~50 stocks)", key="refresh_nifty50_fund")
            if _refresh_nifty_fund or "_nifty50_fund_cache" not in st.session_state:
                with st.spinner("Fetching all 50 constituent fundamentals..."):
                    st.session_state["_nifty50_fund_cache"] = multi_stock_engine.get_nifty50_fundamentals(limit=50)
            _n50_fund = st.session_state.get("_nifty50_fund_cache") or {"status": "UNAVAILABLE", "reason": "Not fetched yet — click refresh."}
            if _n50_fund.get("status") != "AVAILABLE":
                st.warning(_n50_fund.get("reason") or "NIFTY 50 fundamentals unavailable right now.")
            else:
                st.caption(f"Source: **{_n50_fund.get('source')}** · {_n50_fund.get('method')} · "
                           f"Constituents: {_n50_fund.get('constituents')} ({_n50_fund.get('constituent_source')}) · "
                           f"Coverage: {_n50_fund.get('coverage')}/{_n50_fund.get('constituents')} stocks returned data")
                _nf_rows = [
                    ("P/E (mean)", "trailingPE"), ("Forward P/E (mean)", "forwardPE"),
                    ("P/B (mean)", "priceToBook"), ("ROE (mean)", "returnOnEquity"),
                    ("Profit Margin (mean)", "profitMargins"), ("Revenue Growth (mean)", "revenueGrowth"),
                    ("Earnings Growth (mean)", "earningsGrowth"), ("Debt/Equity (mean)", "debtToEquity"),
                    ("Dividend Yield (mean)", "dividendYield"), ("Market Cap (mean)", "marketCap"),
                ]
                _nf_cols = st.columns(5)
                for _i, (_label, _key) in enumerate(_nf_rows):
                    _val = _n50_fund.get(_key + "_mean"); _cov = _n50_fund.get(_key + "_coverage")
                    _text = "N/A"
                    if _val is not None:
                        if _key in {"returnOnEquity", "profitMargins", "revenueGrowth", "earningsGrowth", "dividendYield"}:
                            _text = f"{_val*100:.2f}%"
                        elif _key == "marketCap":
                            _text = f"₹{_val/1e7:,.0f} Cr"
                        else:
                            _text = f"{_val:.2f}"
                    _nf_cols[_i % 5].metric(_label, _text, help=(f"{_cov or 0} of {_n50_fund.get('constituents')} constituents reporting" if _cov is not None else None))
                st.caption(_n50_fund.get("regime_note", ""))
...
def _research_metric(v, prefix="", suffix=""):
    if v is None or v == "": return "N/A"
    try: return f"{prefix}{float(v):,.2f}{suffix}"
    except Exception: return str(v)

def _render_fundamentals_cards(f):
    source = f.get("source", "Unavailable")
    st.caption(f"Fundamental source: **{source}**")
    if f.get("status") != "AVAILABLE":
        st.warning(f.get("reason", "Fundamental data unavailable.")); return
    if source == "Upstox Company Fundamentals":
        ratios = f.get("ratio_map", {})
        profile = f.get("company_profile")
        if profile: st.info(profile)
        ...
        for i,(k,label) in enumerate(labels):
            val=f.get(k); text=_research_metric(val)
            if k in {"returnOnEquity","profitMargins","revenueGrowth","earningsGrowth"} and val is not None:
                try: text=f"{float(val)*100:.2f}%"
                except Exception: pass
            cols[i%4].metric(label,text)

def _render_index_fundamentals_cards(nf):
    """Renders the NIFTY 50 equal-constituent aggregate fundamentals snapshot
    (multi_stock_engine.get_nifty50_fundamentals). Different shape from a
    single company's fundamentals -- these are *_mean / *_coverage fields
    across up to 50 constituents, so it gets its own small renderer instead
    of being forced through _render_fundamentals_cards."""
    nf = nf or {}
    if nf.get("status") != "AVAILABLE":
        st.warning(nf.get("reason") or "NIFTY 50 fundamentals unavailable right now.")
        return
    st.caption(f"Source: **{nf.get('source')}** · {nf.get('method')} · "
               f"Constituents: {nf.get('constituents')} ({nf.get('constituent_source')}) · "
               f"Coverage: {nf.get('coverage')}/{nf.get('constituents')} stocks returned data")
    rows = [
        ("P/E (mean)", "trailingPE"), ("Forward P/E (mean)", "forwardPE"),
        ("P/B (mean)", "priceToBook"), ("ROE (mean)", "returnOnEquity"),
        ("Profit Margin (mean)", "profitMargins"), ("Revenue Growth (mean)", "revenueGrowth"),
    ...
    for i, (label, key) in enumerate(rows):
        val = nf.get(key + "_mean"); cov = nf.get(key + "_coverage")
        text = "N/A"
        if val is not None:
            if key in {"returnOnEquity", "profitMargins", "revenueGrowth", "earningsGrowth", "dividendYield"}:
                text = f"{val*100:.2f}%"
            elif key == "marketCap":
                text = f"₹{val/1e7:,.0f} Cr"
            else:
                text = f"{val:.2f}"
        cols[i % 5].metric(label, text, help=f"{cov or 0} of {nf.get('constituents')} constituents reporting" if cov is not None else None)
    st.caption(nf.get("regime_note", ""))


def _render_stock_news(rows):
    if not rows: st.info("Company news unavailable from the current source."); return
    for n in rows[:8]:
        title=n.get("title") or "Untitled"
        published=n.get("published") or "Time unavailable"
        source=n.get("source") or "Source unavailable"
        link=n.get("link")
        if link: st.markdown(f"- **{title}** — {source} · {published} · [Open]({link})")
        else: st.markdown(f"- **{title}** — {source} · {published}")


# -------------------------------------------------------------
# Shared "clean card" renderers -- used by BOTH the stock dashboard and the
# Gold/Silver panel so every place in the app shows the SAME polished
# style (tables + colour banners) instead of a raw JSON/dict dump.
# -------------------------------------------------------------
def _render_market_structure_smc(ms_list, smc):
    """Nifty-style Smart Money Concepts block: a coloured status banner plus
    3 clean side-by-side tables (FVG / Order Blocks / Liquidity Sweeps) --
    no raw dict/JSON ever shown."""
    if ms_list:
        ms0 = ms_list[0] if isinstance(ms_list, list) else ms_list
        event = str(ms0.get("Market Event", "N/A"))
        trigger = ms0.get("Trigger Level", "N/A")
        if "Bearish" in event:
            st.error(f"**Market Structure Status:** {event}  |  **Trigger Level:** {trigger}")
        elif "Bullish" in event:
            st.success(f"**Market Structure Status:** {event}  |  **Trigger Level:** {trigger}")
        else:
            st.info(f"**Market Structure Status:** {event}  |  **Trigger Level:** {trigger}")
    else:
        st.caption("Market structure: not enough data yet.")

    smc = smc or {}
    fvg_rows = smc.get("fvg") or smc.get("fvgs") or []
    ob_rows = smc.get("order_blocks") or []
    sweep_rows = smc.get("liquidity_sweeps") or []
    fcol, ocol, lcol = st.columns(3)
    with fcol:
        st.markdown("**📌 Fair Value Gaps (FVG)**")
        if fvg_rows:
...
def _render_candle_summary(candle):
    if candle and isinstance(candle, dict):
        st.caption(f"**Latest candle:** {candle.get('pattern', 'N/A')} · Bias: {candle.get('bias', 'N/A')} · Strength: {candle.get('strength', 'N/A')}")


def _render_liquidity_levels(liq):
    """Fixes the earlier bug where a one-line ternary here made Streamlit
...
def _render_macro_table(macro):
    """Clean instrument table instead of st.json() -- same info, professional layout."""
    labels = {"dxy": "DXY (US Dollar Index)", "us10y": "US 10Y Yield", "oil": "Crude Oil (WTI)",
              "copper": "Copper", "sp500": "S&P 500", "gold_silver_ratio": "Gold/Silver Ratio"}
    rows = []
    for key, row in (macro or {}).items():
        if not isinstance(row, dict):
            continue
        label = labels.get(key, key.replace("_", " ").title())
        if row.get("status") == "AVAILABLE":
            last = row.get("last", row.get("value"))
            chg = row.get("change_pct")
            rows.append({
                "Instrument": label,
                "Last": f"{last:,.2f}" if isinstance(last, (int, float)) else "N/A",
                "Change %": f"{chg:+.2f}%" if isinstance(chg, (int, float)) else "N/A",
...
def _render_commodity_fundamentals(f):
    f = f or {}
    if f.get("status") != "STRUCTURAL_AVAILABLE":
        st.info(f.get("reason", "Commodity fundamentals unavailable.")); return
    st.caption(f.get("method", ""))
    drivers = f.get("drivers", [])
    if drivers:
        st.markdown("**Key drivers:** " + ", ".join(str(d) for d in drivers))
    factors = f.get("factor_monitor", {})
    if factors:
...
def _render_trade_candidate(tc, currency_prefix="₹"):
    tc = tc or {}
    status = tc.get("status", "N/A")
    if status == "NO_VALIDATED_SETUP":
        st.info("🕐 Abhi koi validated setup nahi hai — structure confirm hone ka wait karo.")
        return
    if status == "CANDIDATE":
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Direction", tc.get("direction", "N/A"))
        c2.metric("Entry (ref)", _research_metric(tc.get("entry_reference"), currency_prefix))
        c3.metric("Stop Loss", _research_metric(tc.get("stop_loss"), currency_prefix))
        c4.metric("Target", _research_metric(tc.get("target"), currency_prefix))
        st.caption(f"R:R {tc.get('rr', 'N/A')} — {tc.get('reason', '')}")
        return
    st.info(f"Status: {status}")

def _render_stock_dashboard(symbol):
    """The full 'Select Your Stock' dashboard -- same engines, same
    weights, same layout/style as the Nifty 50 dashboard above, built
    entirely from `symbol`'s own real data and `symbol`'s own trade
    history. This is what the sidebar 'Load Stock' button switches to."""
    st.subheader(f"⚡ {symbol} — Full AI Trading Dashboard")
    st.caption("Nifty 50 dashboard jaisi hi saari engines (liquidity, support/resistance, SMC/ICT, market regime, "
               "sniper setup, institutional AI scoring, position sizing, risk, backtest, Monte Carlo) -- bus data "
               f"is neeche {symbol} ka real data hai, Nifty ka nahi.")

    _stock_access = st.session_state.get("access_token")
    _capital = st.session_state.get('risk_capital', 100000.0)
    _risk_pct = st.session_state.get('risk_pct', 1.0)
    _cache_key = f"stock_full_analysis_{symbol}"

    _refresh = st.button(f"🔄 Refresh {symbol} Dashboard", key="refresh_stock_dashboard")
    if _refresh or _cache_key not in st.session_state:
        with st.spinner(f"Running full {symbol} research (same engines as Nifty 50)..."):
            try:
                st.session_state[_cache_key] = multi_stock_engine.analyze_stock_full(
                    symbol, access_token=_stock_access, ai_engine=ai_engine,
                    global_avg_change=st.session_state.get("_last_global_avg_change", 0.0),
                    sentiment_score=st.session_state.get("_last_sentiment_score", "Neutral"),
                    capital=_capital, risk_pct=_risk_pct,
                    live_vix=st.session_state.get("_last_live_vix"),
                )
            except Exception as _e:
                logger.exception("Full stock dashboard failed")
                st.session_state[_cache_key] = {"status": "ERROR", "message": f"{type(_e).__name__}: {_e}"}

    sa = st.session_state.get(_cache_key)
    if not sa:
        st.info("Loading...")
        return
    if sa.get("status") != "OK":
        st.error(sa.get("message", f"{symbol} ka data abhi available nahi hai. Symbol sahi check karo (NSE tradingsymbol, jaise RELIANCE, TCS, HDFCBANK)."))
        return

    ai_res = sa.get("ai_result")
    st.success(f"Primary research target: **{sa['symbol']}** ({sa.get('company')}) · Market data: {sa['data_source']}")
    c1, c2, c3, c4, c5 = st.columns(5)
    c1.metric(f"Real-Time Live {symbol} Price (LTP)", _research_metric(sa['live_price'], "₹"))
    c2.metric("Institutional Confluence Signal", ai_res["signal_type"] if ai_res else sa["decision"])
    c3.metric("AI Confluence Score (live heuristic)", f"{ai_res['confidence_pct']}%" if ai_res else "N/A")
    c4.metric("RSI", _research_metric(sa["rsi"]))
    c5.metric("Trend", sa["trend"])
    st.caption(f"Market source: **{sa['data_source']}** · Candle: **{sa['timestamp']}** · ISIN: **{sa.get('isin') or 'N/A'}**")
    if sa.get("full_dashboard_status") and sa["full_dashboard_status"] != "OK":
        st.warning(sa["full_dashboard_status"])

    if ai_res:
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown("#### 🎯 AI Trade Signal")
        st.caption("Nifty 50 dashboard wala hi model aur wahi weights: Technical 20% + SMC/ICT 25% + Macro 15% + "
                   f"Institutional Flow 40%. Sirf inputs {symbol} ke apne real data (apna price/volume/order-book/sector) se hain.")
        if ai_res.get("has_setup"):
            a1, a2, a3, a4 = st.columns(4)
            a1.metric("Bias", ai_res["bias_text"])
            a2.metric("Entry", _research_metric(ai_res["entry_price"], "₹"))
            a3.metric("Stop Loss", _research_metric(ai_res["stop_loss"], "₹"))
            a4.metric("Target 1 / 2", f"₹{ai_res['target_1']:,.2f} / ₹{ai_res['target_2']:,.2f}")
            st.caption(f"Net score {ai_res['net_score']} | Tech {ai_res['tech_score']} | SMC {ai_res['smc_score']} | "
                       f"Macro {ai_res['macro_score']} | Flow {ai_res['flow_score']} | R:R {ai_res['risk_reward_ratio']}")
        else:
            st.info(f"⏳ **Abhi koi genuine setup nahi hai** — Bias: **{ai_res['bias_text']}**. "
                    "Jab tak confluence directionally strong nahi ho jaata (sideways/choppy read), "
                    "engine koi Entry/SL/Target nahi dega — apne mann se trade nahi thopega.")
            st.caption(f"Net score {ai_res['net_score']} | Tech {ai_res['tech_score']} | SMC {ai_res['smc_score']} | "
                       f"Macro {ai_res['macro_score']} | Flow {ai_res['flow_score']}")

    tabs = st.tabs(["Overview", "Technical + SMC/ICT", "Regime + CPR + Sniper", "Options + Order Flow",
                     "Position + Risk + Backtest", "Fundamentals", "News + Context"])

    with tabs[0]:
        a, b, c, d = st.columns(4)
        a.metric("Support", _research_metric(sa.get('support'), "₹")); b.metric("Resistance", _research_metric(sa.get('resistance'), "₹"))
        c.metric("EMA 20", _research_metric(sa.get('ema20'), "₹")); d.metric("EMA 50", _research_metric(sa.get('ema50'), "₹"))
        st.markdown("#### Market decision context")
        q = sa.get("quote", {}); tc = sa.get("technical_context", {}); mtf = tc.get("mtf", {})
    ...
    with tabs[1]:
        ...
        st.markdown("#### Multi-timeframe structure — selected stock only")
        _render_mtf_table(tc.get("mtf", {}))

        st.markdown("---")
        st.markdown("#### 🏛️ Smart Money Concepts & Market Structure (BOS / CHoCH)")
        _render_market_structure_smc(tc.get("market_structure", []), tc.get("smc", {}))
        _render_candle_summary(tc.get("candlestick", {}))

        st.markdown("---")
        st.markdown("#### 💧 Liquidity map")
        _render_liquidity_levels(tc.get("liquidity", {}))

    with tabs[2]:
        regime = sa.get("regime", {}) or {}
        ...
        if regime.get('adx') is not None:
            st.caption(f"ADX {regime['adx']} (+DI {regime.get('plus_di')}/-DI {regime.get('minus_di')}) | "
                       f"ATR percentile {regime.get('atr_percentile')} | VIX {regime.get('vix')}")
        st.write(sa.get("regime_guidance", ""))
        st.markdown("---")
        pivots = sa.get("pivots", {}) or {}
        if pivots.get("status") == "OK":
            st.markdown(f"**Today's Pivot:** ₹{pivots['pivot']:,.2f} | **CPR:** ₹{pivots['bc']:,.2f}–₹{pivots['tc']:,.2f} "
                        f"({pivots['cpr_label']}) | **Position:** {sa.get('cpr_position')}")
            st.caption(f"R1 {pivots['r1']:,.2f} | R2 {pivots['r2']:,.2f} | R3 {pivots['r3']:,.2f}  ·  "
                       f"S1 {pivots['s1']:,.2f} | S2 {pivots['s2']:,.2f} | S3 {pivots['s3']:,.2f}  ·  "
                       f"PDH {pivots['pdh']:,.2f} | PDL {pivots['pdl']:,.2f} | PDC {pivots['pdc']:,.2f}")
        else:
            st.caption("Pivots: not enough prior-day data yet.")
        orb = sa.get("opening_range", {}) or {}
        if orb.get("status") == "OK":
            st.markdown(f"**Opening Range ({orb['orb_minutes']}m):** ₹{orb['orb_low']:,.2f} – ₹{orb['orb_high']:,.2f} → {orb['read']}")
        st.markdown("---")
        st.markdown("#### 🎯 Sniper Setup")
        sniper = sa.get("sniper")
        if sniper:
            st.write(f"**Bias:** {sniper['setup_bias']}")
            st.write(f"**Key Levels:** {sniper['key_levels']}")
            st.caption(sniper['cpr_note'])
            st.write(f"**Price Action Confluence:** {sniper['price_action_confluence']}")
            st.write(f"**Option Chain Validation (\"90% Rule\"):** {sniper['oi_validation']}")
            st.write(f"**Execution Trigger:** {sniper['execution_trigger']}")
            if sniper.get('trade_plan'):
                tp = sniper['trade_plan']
                st.success(f"Trade Plan — Entry ₹{tp['entry']:,.2f} | Target ₹{tp['target']:,.2f} | SL ₹{tp['sl']:,.2f}")
            else:
                st.info("Trade Plan: No trade — confluence not confirmed yet.")
        else:
            st.caption("Sniper setup unavailable (not enough candle history yet).")
    ...
    with tabs[4]:
        st.markdown("**Position Sizing Calculator**")
        ps = sa.get("position_size", {}) or {}
        if ps.get("status") == "OK":
            st.write(f"Lots: **{ps['lots']}** ({ps['quantity']} qty) | SL Distance: {ps['sl_distance_pts']} pts | "
                     f"Risk: ₹{ps['actual_risk_rs']:,.0f} ({ps['actual_risk_pct_of_capital']}% of capital)")
            for w in ps.get("warnings", []): st.caption(f"⚠️ {w}")
        else:
            st.caption(ps.get("reason", "Position sizing unavailable."))
        st.markdown("---")
        st.markdown(f"**Risk Engine** ({symbol}'s own logged trades — {sa.get('trade_history_count', 0)} resolved so far)")
        rs = sa.get("risk_state", {}) or {}
        st.write(f"Status: **{rs.get('status', 'N/A')}**")
        if rs.get("today_trade_count") is not None:
            st.write(f"Today: {rs['today_trade_count']} trades | {rs['today_pnl_pts']} pts | Consecutive losses: {rs.get('consecutive_losses')}")
        for reason in rs.get("block_reasons", []): st.caption(f"🛑 {reason}")
        st.markdown("---")
        st.markdown(f"**Real Backtest Report** ({symbol}'s own logged trades only — never mixed with Nifty's or another stock's)")
        bt = sa.get("backtest_report", {}) or {}
        if bt.get("status") == "OK":
            bc1, bc2, bc3 = st.columns(3)
            bc1.metric("Win Rate", f"{bt['win_rate_pct']}%"); bc1.metric("Profit Factor", bt['profit_factor'])
            bc2.metric("Expectancy/Trade", f"{bt['expectancy_pts_per_trade']} pts"); bc2.metric("Avg R-Multiple", bt['avg_r_multiple'])
            bc3.metric("Max Drawdown", f"{bt['max_drawdown_pts']} pts"); bc3.metric("Longest Losing Streak", bt['longest_losing_streak'])
        else:
            st.caption(bt.get("note", f"Not enough {symbol} trade history yet — log a few paper trades on this stock to unlock this."))
        st.markdown("---")
        st.markdown("**Monte Carlo Analysis** (bootstrap of this stock's real R-multiples)")
        mc = sa.get("monte_carlo_report", {}) or {}
        if mc.get("status") == "OK":
            mc1, mc2 = st.columns(2)
            mc1.metric("Median Max Drawdown", f"{mc['max_drawdown_r_median']} R")
            mc2.metric("Risk of Ruin", f"{mc['risk_of_ruin_pct_of_sims']}%")
        else:
            st.caption(mc.get("note", "Monte Carlo unavailable."))
        st.markdown("---")
        st.markdown("**Model Drift Detection**")
        dr = sa.get("drift_report", {}) or {}
        st.write(f"Status: {dr.get('status', 'N/A')}")
        for reason in dr.get('drift_reasons', []): st.caption(f"⚠️ {reason}")

    with tabs[5]:
        _render_fundamentals_cards(sa.get("fundamentals", {}))

    with tabs[6]:
        st.markdown("#### Company news")
        _render_stock_news(sa.get("news", []))
        st.markdown("#### Sector + Nifty 50 context")
        st.dataframe(pd.DataFrame([{
            "Sector": sa.get("sector"), "Sector Change %": sa.get("sector_context", {}).get("change_pct"),
        ...
        _render_index_fundamentals_cards(sa.get("nifty_fundamentals", {}))

    # Keep the complete selected-stock snapshot available to the AI chatbox.
    _ctx = st.session_state.setdefault("dashboard_context", {})
    _ctx["selected_stock_analysis"] = sa
    _ctx["research_target"] = sa["symbol"]


if ACTIVE_SYMBOL:
    try:
        _render_stock_dashboard(ACTIVE_SYMBOL)
    except Exception as _stock_ui_err:
        logger.exception("Full stock dashboard failed")
        st.error(f"Stock dashboard error: {_stock_ui_err}")
else:
    st.subheader("📊 Select Your Stock")
    st.caption("👈 Sidebar mein koi bhi NSE stock ka naam daal kar **Load Stock** dabao — poora dashboard "
               "us stock ka ban jaayega, upar Nifty 50 dashboard jaisi hi saari engines/style ke saath.")

# -------------------------------------------------------------
# GOLD / SILVER COMMODITY INTELLIGENCE
# Same research workflow as equities, with commodity-specific
# fundamentals/macro drivers and isolated learning profiles.
# -------------------------------------------------------------
st.markdown("---")
st.subheader("🥇 Gold & Silver AI Intelligence — Forex Spot")
st.caption("Primary instruments: XAU/USD (Gold) and XAG/USD (Silver), quoted in USD per troy ounce. MCX is kept separate. Gold/Silver use the same technical + risk + learning framework, but fundamentals are commodity fundamentals: supply/demand, central-bank/investment flows, industrial demand, DXY, yields, rates, inflation and geopolitical risk. No company-style PE/EPS is fabricated.")
try:
    _commodity_asset = st.selectbox("Select commodity", ["GOLD", "SILVER"], key="commodity_asset_selector")
    if st.button("🔎 Analyze Gold/Silver", key="analyze_commodity"):
        with st.spinner(f"Analyzing {_commodity_asset}..."):
            st.session_state["selected_commodity_analysis"] = commodity_engine.analyze_commodity(_commodity_asset, access_token=st.session_state.get("access_token"))
    _ca = st.session_state.get("selected_commodity_analysis")
    if _ca and _ca.get("asset") == _commodity_asset:
        if _ca.get("status") != "OK":
            st.error(_ca.get("message", "Commodity data unavailable."))
        else:
            _ct = _ca["technical"]
            # One-line summary, always visible. Tap it to expand the full
            # data below; tap again to collapse it back to this one line.
            _summary_line = (
                f"**{_commodity_asset}** {_ct['live_price']:,.2f} · Trend: {_ct['trend']} · "
                f"RSI: {_ct['rsi']:.1f} · AI Bias: {_ca['decision']}"
            )
            with st.expander(_summary_line, expanded=False):
                c1,c2,c3,c4 = st.columns(4)
                c1.metric("XAU/XAG Spot Price", f"{_ct['live_price']:,.2f}")
                c2.metric("Trend", _ct["trend"])
                c3.metric("RSI", f"{_ct['rsi']:.1f}")
                c4.metric("AI Bias", _ca["decision"])
                st.caption(f"Source: {_ca['data_source']} | Unit: {_ca['unit']} | Candle: {_ct['timestamp']}")
                t1,t2,t3,t4 = st.columns(4)
                t1.metric("Support", f"{_ct['support']:,.2f}" if _ct['support'] else "N/A")
                t2.metric("Resistance", f"{_ct['resistance']:,.2f}" if _ct['resistance'] else "N/A")
                t3.metric("EMA 20", f"{_ct['ema20']:,.2f}")
                t4.metric("EMA 50", f"{_ct['ema50']:,.2f}")
                st.markdown("### Commodity Fundamentals")
                _render_commodity_fundamentals(_ca.get("fundamentals", {}))

                st.markdown("### Macro / Cross-Market Context")
                _render_macro_table(_ca.get("macro_context", {}))

                st.markdown("### Structural / Liquidity / Regime Context")
                _smc_ctx = _ca.get("smc_context", {}) or {}
                _render_market_structure_smc(_smc_ctx.get("market_structure", []), _smc_ctx)
                _regime_ctx = _ca.get("market_regime", {}) or {}
                st.markdown(f"**Regime:** {_regime_ctx.get('primary','N/A')} | **Volatility:** {_regime_ctx.get('volatility','N/A')} | **Structure:** {_regime_ctx.get('structure','N/A')}")
                st.markdown("**Liquidity map**")
                _render_liquidity_levels(_ca.get("liquidity_context", {}))
                st.markdown("**Multi-timeframe structure**")
                _render_mtf_table(_ca.get("multi_timeframe", {}))

                st.markdown("### AI Trade Candidate")
                _render_trade_candidate(_ca.get("trade_candidate", {}), currency_prefix="$")

                st.markdown("### Latest Commodity News")
                if _ca.get("news"):
                    for _n in _ca["news"]:
                        st.markdown(f"- **{_n['title']}** — {_n['published']}")
                else:
                    st.info("Commodity news unavailable from the current RSS source.")
                st.markdown("### Analysis Summary")
                st.write(f"Technical trend: **{_ct['trend']}**. Momentum: **{_ct['momentum']}**. Bullish context score: **{_ca['bullish_score']}**, bearish context score: **{_ca['bearish_score']}**. This is an analytical bias, not a guaranteed future price.")
            # Make the complete latest commodity snapshot available to the AI chatbox.
            _ctx = st.session_state.setdefault("dashboard_context", {})
            _ctx["selected_commodity_analysis"] = _ca
            _ctx["commodity_asset"] = _commodity_asset
except Exception as _commodity_ui_err:
    logger.exception("Commodity panel failed")
    st.error(f"Gold/Silver analysis error: {_commodity_ui_err}")
