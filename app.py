...
# itself every 30s WITHOUT rerunning the rest of the script.
# -------------------------------------------------------------
...
def _live_dashboard():
    ...
    if df is not None and not df.empty:
        ...
        # 🕒 REAL MARKET STATE + DATA FRESHNESS CHECK
        ...
        st.caption("Ye level ko TOUCH hone se pehle fire hota hai — support/resistance ke paas pahunchte hi break ya bounce hone ka % chance dikhata hai.")
        ...
        if level_prediction['status'] == 'APPROACHING_LEVEL':
            ...
            st.info(f"Price abhi kisi key support/resistance ke approach-zone me nahi hai. Nearest Support: {ns_text} | Nearest Resistance: {nr_text}")
...
# Run live dashboard fragment
_live_dashboard()

st.markdown("---")

# -------------------------------------------------------------
# AI CHAT RENDERED OUTSIDE THE FRAGMENT (Prevents refresh clearing)
# -------------------------------------------------------------
