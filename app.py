...
def _live_dashboard():
    ...
    if df is not None and not df.empty:
        ...
        # -------------------------------------------------------------
        next_move = next_move_predictor.predict_next_5min_direction(
            ...
            india_news_sentiment=india_news_sentiment
        )
        st.subheader("🎯 Next 5-Minute Direction Call")
        nm1, nm2 = st.columns([1, 2])
        with nm1:
            if next_move['direction'] == "UP":
                st.success(f"### ⬆️ UP\n**{next_move['confidence_pct']}%** confidence")
            elif next_move['direction'] == "DOWN":
                st.error(f"### ⬇️ DOWN\n**{next_move['confidence_pct']}%** confidence")
            else:
                st.warning(f"### ↔️ SIDEWAYS\nNo clear edge ({next_move['confidence_pct']}%)")
        with nm2:
            st.caption(f"Votes weighed: {next_move['votes_up']} up vs {next_move['votes_down']} down "
                       f"(total weight {next_move['votes_total_weight']}).")
            if next_move['caution']:
                st.caption(f"⚠️ {next_move['caution']}")
            st.caption("⚠️ Ye ek weighted probability lean hai, guarantee nahi — koi bhi tool 5-minute index "
                       "direction 100% accurate call nahi kar sakta. Apna risk khud manage karo, position size "
                       "chhota rakho, aur stop-loss zaroor lagao.")
