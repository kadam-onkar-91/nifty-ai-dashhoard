...
# Page Configuration
st.set_page_config(page_title="Institutional AI Trading Dashboard", page_icon="⚡", layout="wide")
...
def _live_dashboard():
    ...
    # -------------------------------------------------------------
    # SELECTED STOCK MODE
    # After Upstox login, this single box becomes the primary research target.
    # Once a symbol is entered, the complete dashboard switches to that
    # stock's own live Upstox instrument. NIFTY-specific price/options/futures
    # data is NOT used as a substitute anywhere in stock mode.
    # -------------------------------------------------------------
    if access_token:
        ...
        stock_input = st.text_input(
            ...
            placeholder="RELIANCE, TCS, HDFCBANK, SBIN…",
            key="selected_stock_input",
            label_visibility="collapsed",
            help="NSE trading symbol type karo. Example: RELIANCE, TCS, HDFCBANK."
        ...
        if stock_input:
            ...
            with bc1:
                if st.button("↩️ NIFTY 50", key="back_to_nifty_from_stock"):
                    st.session_state["selected_stock"] = ""
                    st.session_state["selected_stock_input"] = ""
                    st.rerun()
            with bc2:
                st.caption(f"Full-screen asset mode: **{stock_input}** — NIFTY dashboard ki same methodology/style, lekin asset-specific live data.")
            with st.spinner(f"{stock_input} ka live institutional dashboard load ho raha hai…"):
                stock_result = multi_stock_engine.analyze_stock_full(
                    stock_input, access_token, ai_engine=ai_engine
                )
            if stock_result.get("status") == "OK":
                stock_dashboard.render_stock_dashboard(stock_result)
                return
            elif stock_result.get("status") in {"INVALID", "DATA_UNAVAILABLE"}:
                st.error(f"⚠️ {stock_result.get('message', 'Selected stock ka live data available nahi hai.')}")
                st.caption("Trading analysis ko NIFTY/Yahoo data se fill nahi kiya gaya — source unavailable ho to dashboard us factor ko unavailable hi rakhta hai.")
                return

    # Step 2: Fetch Live NIFTY Market Data
    # If no stock is selected, render the original NIFTY dashboard.
    st.title("⚡ Institutional AI Trading Dashboard (Pro Edition)")
    st.markdown("---")
...
# When a stock is selected, its dashboard is intentionally full-screen.
# The older Universal Asset Research / Commodity / generic AI-chat panels
# are hidden so no second asset or NIFTY context can appear below it.
...
st.markdown("<br>", unsafe_allow_html=True)
st.markdown("---")
st.subheader("📊 Universal Asset Research")
st.caption("Search any NSE stock. The selected asset becomes the primary research target; NIFTY/global data remain context only. Live Upstox data is preferred and every fallback is explicitly labelled.")
...
try:
    _stock_options = multi_stock_engine.NSE_STOCKS
    _watch = st.session_state.setdefault("custom_stock_watchlist", [])
    _search_col, _select_col, _add_col = st.columns([1.4, 1.4, 0.7])
    with _search_col:
        _typed_symbol = st.text_input("🔎 Search NSE stock", placeholder="HDFCBANK / RELIANCE / TCS", key="custom_stock_search").strip().upper()
    with _select_col:
        _stock_symbol = st.selectbox("Or choose stock", _stock_options, index=0, key="multi_stock_symbol")
    _chosen_symbol = _typed_symbol if _typed_symbol else _stock_symbol
    with _add_col:
        st.write("")
        if st.button("➕ Add", key="add_custom_stock") and _chosen_symbol:
            import re as _re
            if _re.fullmatch(r"[A-Z0-9&._-]{2,30}", _chosen_symbol) and _chosen_symbol not in _watch:
                _watch.append(_chosen_symbol)
    if _watch:
        st.caption("Watchlist: " + ", ".join(_watch))
    _stock_access = st.session_state.get("access_token")
    if st.button(f"🔎 Run Full {_chosen_symbol} Research", key="analyze_selected_stock"):
        with st.spinner(f"Running {_chosen_symbol} full research..."):
            st.session_state["selected_stock_analysis"] = multi_stock_engine.analyze_stock(_chosen_symbol, _stock_access)
    _sa = st.session_state.get("selected_stock_analysis")
    if _sa and _sa.get("symbol") == _chosen_symbol:
        if _sa.get("status") != "OK":
            st.error(_sa.get("message", "Stock data unavailable."))
        else:
            st.success(f"Primary research target: {_sa['symbol']} · Market data: {_sa['data_source']}")
            c1,c2,c3,c4,c5 = st.columns(5)
            c1.metric("LTP", _research_metric(_sa['live_price'], "₹"))
            c2.metric("Trend", _sa["trend"])
            c3.metric("RSI", _research_metric(_sa["rsi"]))
            c4.metric("VWAP", _research_metric(_sa.get("technical_context",{}).get("vwap"), "₹"))
            c5.metric("Research Bias", _sa["decision"])
            st.caption(f"Market source: **{_sa['data_source']}** · Candle: **{_sa['timestamp']}** · ISIN: **{_sa.get('isin') or 'N/A'}**")

            tabs=st.tabs(["Overview","Technical + SMC/ICT","Options + Order Flow","Fundamentals","News + Context"])
            with tabs[0]:
                a,b,c,d=st.columns(4)
                a.metric("Support", _research_metric(_sa.get('support'), "₹")); b.metric("Resistance", _research_metric(_sa.get('resistance'), "₹")); c.metric("EMA 20", _research_metric(_sa.get('ema20'), "₹")); d.metric("EMA 50", _research_metric(_sa.get('ema50'), "₹"))
                st.markdown("#### Market decision context")
                q=_sa.get("quote",{}); tc=_sa.get("technical_context",{}); mtf=tc.get("mtf",{})
                st.dataframe(pd.DataFrame([{
                    "Trend":_sa["trend"],"Momentum":_sa["momentum"],"MTF Alignment":mtf.get("alignment"),
                    "Order-book":q.get("pressure","N/A"),"OBI %":q.get("imbalance_pct"),"Sector":_sa.get("sector"),"NIFTY Change %":_sa.get("nifty_context",{}).get("nifty_change_pct")
                }]), hide_index=True, use_container_width=True)
            with tabs[1]:
                tc=_sa.get("technical_context",{})
                st.markdown("#### Multi-timeframe structure — selected stock only")
                mtf_rows=[]
                for tf,v in tc.get("mtf",{}).items():
                    if isinstance(v,dict): mtf_rows.append({"Timeframe":tf,"Status":v.get("status"),"Trend":v.get("trend","N/A"),"Structure":v.get("structure","N/A"),"Bars":v.get("bars","N/A")})
                ...
                liq=tc.get("liquidity",{}); st.dataframe(pd.DataFrame(liq.get("levels",[])[:12]), hide_index=True, use_container_width=True) if liq.get("levels") else st.info("No liquidity levels available.")
            with tabs[2]:
                oc=_sa.get("option_chain",{})
                if oc.get("status")=="AVAILABLE":
                    ...
                    st.info(oc.get("reason", "No live stock option chain available for this underlying."))
                q=_sa.get("quote",{})
                st.markdown("#### Live order-book snapshot")
                st.dataframe(pd.DataFrame([q]), hide_index=True, use_container_width=True)
            with tabs[3]:
                _render_fundamentals_cards(_sa.get("fundamentals",{}))
            with tabs[4]:
                st.markdown("#### Company news")
                _render_stock_news(_sa.get("news",[]))
                st.markdown("#### NIFTY 50 context")
                nf=_sa.get("nifty_fundamentals",{})
                if nf.get("status")=="AVAILABLE": st.caption(f"{nf.get('source')} · Coverage {nf.get('coverage')}/{nf.get('constituents')} · {nf.get('method')}")
                st.markdown("#### Sector + market context")
                st.dataframe(pd.DataFrame([{"Sector":_sa.get("sector"),"Sector Change %":_sa.get("sector_context",{}).get("change_pct"),"NIFTY Change %":_sa.get("nifty_context",{}).get("nifty_change_pct")}]), hide_index=True, use_container_width=True)
                st.markdown("#### NIFTY news")
                _render_stock_news(_sa.get("nifty_news",[]))

            # Keep the complete selected-stock snapshot available to the AI chatbox.
            _ctx = st.session_state.setdefault("dashboard_context", {})
            _ctx["selected_stock_analysis"] = _sa
            _ctx["research_target"] = _sa["symbol"]
except Exception as _stock_ui_err:
    logger.exception("Multi-stock panel failed")
    st.error(f"Stock research error: {_stock_ui_err}")
