...
def get_nifty_futures_instrument_key():
    """Public wrapper -- PHASE 3's order_flow.py needs the same near-month
    NIFTY futures instrument_key this module already resolves for real
    volume, so it's exposed here instead of duplicating the instrument
    master lookup logic in a second file."""
...
def fetch_nifty_futures_volume(access_token):
    ...
    if not access_token:
        return None
    fut_key, _debug = _load_upstox_nifty_future_key()
    if not fut_key:
        return None
